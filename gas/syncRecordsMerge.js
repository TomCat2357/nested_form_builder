...
function Sync_getFixedMetaColumnValue_(record, key, toUnixMs) {
  ...
  if (key === "createdAt" || key === "modifiedAt" || key === "deletedAt") {
    // シート上は数値の日時 (Date オブジェクト) で格納する。
    // 入力優先順: record[key] (canonical 文字列 / Date / Unix ms) → record[key+'UnixMs'] (シム)
    var rawValue = record[key];
    var unixMs = null;
    if (rawValue !== null && rawValue !== undefined && rawValue !== "") {
      var direct = toUnixMs(rawValue);
      if (typeof direct === "number" && isFinite(direct) && direct > 0) unixMs = direct;
    }
    if (!(typeof unixMs === "number" && isFinite(unixMs) && unixMs > 0)) {
      var shim = parseInt(record[key + "UnixMs"], 10);
      if (isFinite(shim) && shim > 0) unixMs = shim;
    }
    if (typeof unixMs === "number" && isFinite(unixMs) && unixMs > 0) {
...
function Sync_syncFixedMetaColumnsFromRecord_(params) {
  ...
  // メタ日時列 (createdAt/modifiedAt/deletedAt) はシート上は数値の日時 (Date) を格納するため
  //   numberFormat を yyyy/mm/dd hh:mm:ss に。No. 列は数値のままなので `0`。
  var metaDtFmt = (typeof NFB_SHEETS_DATETIME_FORMAT !== "undefined") ? NFB_SHEETS_DATETIME_FORMAT : "yyyy/mm/dd hh:mm:ss";
  var baseSpecs = mode === "overwrite"
    ? [
      { key: "No.", colIdx: resolveColIdx("No.", 1), numberFormat: "0" },
      { key: "createdAt", colIdx: resolveColIdx("createdAt", 2), numberFormat: metaDtFmt },
      { key: "modifiedAt", colIdx: resolveColIdx("modifiedAt", 3), numberFormat: metaDtFmt },
      { key: "deletedAt", colIdx: resolveColIdx("deletedAt", 4), numberFormat: metaDtFmt },
