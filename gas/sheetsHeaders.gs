...
function Sheets_generateRecordId_() {
  ...
  if (nowMs === NFB_LAST_ID_TS_MS) NFB_LAST_ID_SEQ += 1;
  else {
    NFB_LAST_ID_TS_MS = nowMs;
    NFB_LAST_ID_SEQ = 0;
  }
  var tsPart = nowMs.toString(36).padStart(10, "0");
  var seqPart = NFB_LAST_ID_SEQ.toString(36).padStart(4, "0");
  return "r_" + tsPart + "_" + seqPart;
}
