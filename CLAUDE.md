...
```
{TOKEN_NAME}                    # 予約トークンまたはフィールド参照
{fieldLabel|transform:args}     # パイプ変換付き
{field|tr1:args|tr2:args}       # 変換チェーン（左→右）
{\fieldLabel}                   # バックスラッシュで強制フィールド参照（予約トークン名と重複時）
```
...
| `{HH}`, `{mm}`, `{ss}` | 時分秒 |
| `{gg}`, `{ee}` | 和暦元号名・年 |
| `{_PDF}`, `{_DOCUMENT}` | 出力タイプ識別子 |
| `{_folder_url}`, `{_record_url}`, `{_form_url}` | URL参照 |

### パイプ変換

| 変換 | 説明 | 例 |
|------|------|-----|
| `date:format` | 日付書式（YYYY, MM, DD, M, D, YY, gg, gge, ddd, dddd） | `{field\|date:YYYY年M月D日}` |
| `time:format` | 時刻書式 | `{field\|time:H時m分}` |
| `left:n`, `right:n` | 文字列切り出し | `{field\|left:3}` |
| `mid:start:length` | 中間切り出し | `{field\|mid:2:5}` |
| `pad:n`, `padRight:n` | ゼロ埋め | `{field\|pad:5}` |
| `upper`, `lower`, `trim` | 文字列操作 | |
| `default:fallback` | 空値時のデフォルト | `{field\|default:未入力}` |
| `replace:/pattern/repl` | 正規表現置換 | |
| `match:n` | グループ抽出 | |
| `number:format` | 数値書式（桁区切り、小数） | |
| `if:cond:true:false` | 条件分岐 | |
| `map:k1=v1,k2=v2` | 値マッピング | |
