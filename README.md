...
Google Doc テンプレート、ファイル／フォルダ名、Gmail 本文、項目ラベル等で使用するトークン置換システムです。レコードデータを埋め込んだ PDF・Gmail 下書き・Google Doc を自動生成できます。バックエンド実装は `gas/driveTemplate.gs`、フロントエンドのプレビュー用実装は `builder/src/utils/tokenReplacer.js` / `tokenTransformers.js` にあります（サブテンプレート・`{_}`/`{@_}` 参照など一部の高度機能は GAS 側のみ）。
...
`{@fieldLabel}` はフォームに存在する質問ラベル（トップレベル一意）で参照します。ネストされた質問の場合、最上位のラベルが優先されます。
...
> 旧 `{@_folder_url}` / `{@_file_urls}` は廃止されました。fileUpload 欄ごとに後述の `|folder_url` / `|file_urls` パイプを使用します。

### fileUpload 欄専用パイプ

fileUpload フィールドのラベルを `@` 参照した上で以下のパイプを付けると、その欄のファイル情報を取得できます。
...
サブテンプレートは `ifv` の真/偽値、`if` の elseValue、`default` のフォールバック値で使えます。`{_}` / `{@_}` はサブテンプレート内でパイプ入力値を参照します。コンマ分割はブレースを考慮するため、`{...}` 内のコンマでは誤分割されません。
