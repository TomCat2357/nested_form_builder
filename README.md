...
| `if` | `condition,trueValue,falseValue` | 3 項条件分岐。真なら `trueValue`、偽なら `falseValue`（後述） | `{@区分\|if:@区分=="済",完了,未完了}` |
...
### 値位置で使える特殊記法（`if` の trueValue / falseValue、`default` の fallback）
...
サブテンプレートは `if` の trueValue / falseValue、`default` のフォールバック値で使えます。`{_}` / `{@_}` はサブテンプレート内でパイプ入力値を参照します。コンマ分割はブレースを考慮するため、`{...}` 内のコンマでは誤分割されません。

```
{@報告|if:@報告,({_})記載あり,記載なし}
{@状態|if:@状態=="完了",({@対応者})完了,未完了}
...
{@区分|if:@区分=="済",完了,未完了|upper}        # 条件分岐→大文字
