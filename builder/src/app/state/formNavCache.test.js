...
test("dbHelpers v10→最新 upgrade: 既存ストアを温存し formNavCache / analyticsCrossSearches を加算する（非破壊）", async () => {
  const { cleanup, seedDb, databases } = installMockIndexedDB();
  try {
    const { DB_NAME, DB_VERSION, STORE_NAMES } = await import("../../core/constants.js");
    seedDb(DB_NAME, 10, {
    ...
    assert.equal(seeded.stores.has(STORE_NAMES.analyticsCrossSearches), true, "analyticsCrossSearches を加算");
