  ...
  return (
    ...
    >
      <div className="nf-card nf-mb-24">
        ...
        <div className="nf-card nf-mb-16">
          <div className="nf-settings-group-title nf-mb-12">リンクフォーム</div>
          <p className="nf-text-11 nf-text-muted nf-mb-12">レコード詳細画面のサイドバーに表示される関連フォームへのリンクを設定します</p>
          {(localSettings.linkedForms || []).map((link, index) => {
            const otherForms = forms.filter((f) => f.id !== formId && !f.archived);
            return (
              <div key={index} className="linked-form-row nf-mb-8">
                <select
                  className="nf-input linked-form-select"
                  value={link.formId || ""}
                  onChange={(event) => {
                    const updated = [...(localSettings.linkedForms || [])];
                    const selectedForm = forms.find((f) => f.id === event.target.value);
                    updated[index] = {
                      ...updated[index],
                      formId: event.target.value,
                      label: updated[index].label || selectedForm?.settings?.formTitle || "",
                    };
                    handleSettingsChange("linkedForms", updated);
                  }}
                  disabled={isReadLocked}
                >
                ...
                <input
                  className="nf-input linked-form-label"
                  type="text"
                  value={link.label || ""}
                  placeholder="表示ラベル"
                  onChange={(event) => {
                ...
                <button
                  type="button"
                  className="nf-btn-outline nf-text-13 linked-form-remove"
                  onClick={() => {
                    const updated = (localSettings.linkedForms || []).filter((_, i) => i !== index);
                    handleSettingsChange("linkedForms", updated);
                  }}
                  disabled={isReadLocked}
                >
          ...
          <button
            type="button"
            className="nf-btn-outline nf-text-13 nf-mt-8"
            onClick={() => {
              const updated = [...(localSettings.linkedForms || []), { formId: "", label: "" }];
              handleSettingsChange("linkedForms", updated);
            }}
            disabled={isReadLocked}
          >
