...
// listActionsShared.js は jszip を静的 import するため、依存未インストール環境
// （`npm run builder:install` 前）ではこの import が throw する。動的ロードして
// ファイルごとクラッシュ（exitCode 1）させず、未導入なら skip 理由を立てて緑を保つ。
let sanitizeFileBaseName;
let resolveDialogTargetIds;
let createFolderCreateActions;
let createRenameActions;
let skip = false;
try {
  ({
    ...
    createRenameActions,
  } = await import("./listActionsShared.js"));
} catch (err) {
  skip = "jszip 未インストール。`npm run builder:install` 後に実行されます: " + (err && err.message ? err.message : String(err));
}
...
test("sanitizeFileBaseName: 使用不可文字を _ に / 空なら fallback", { skip }, () => {
...
test("resolveDialogTargetIds: targetIds 優先 → idKey → []", { skip }, () => {
...
test("createFolderCreateActions.confirmCreateFolder: 空名はエラー、有効名は currentPath 配下を作成", { skip }, async () => {
  const calls = [];
...
test("createFolderCreateActions.confirmCreateFolder: 失敗時は onError とエラー表示", { skip }, async () => {
  let errored = null;
  let folderError = "";
...
test("createRenameActions.confirmRename: item は renameItem を呼び選択解除", { skip }, async () => {
  let renamed = null;
  let cleared = null;
  const dialog = makeDialog();
...
test("createRenameActions.confirmRename: folder は / 禁止・同名衝突をブロック、正常時 renameFolder", { skip }, async () => {
  const dialog = makeDialog();
