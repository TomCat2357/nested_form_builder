...
test("buildDashboardPayload: 一覧に無い質問でも既存 questionPath を温存（空で上書きしない）", () => {
  const out = buildDashboardPayload({
    dashboard: {
      name: "D",
      cards: [
        // Q1 は一覧にあり → 再計算で更新
        { id: "c1", type: "chart", questionId: "Q1", questionPath: "旧/パス" },
        // アーカイブ/未ロードで一覧に無い → 既存の論理パスを温存
        { id: "c2", type: "chart", questionId: "ARCHIVED", questionPath: "相談/集計表" },
        // 一覧に無く既存パスも無い → 空のまま
        { id: "c3", type: "chart", questionId: "MISSING" },
      ],
    },
    questions: [{ id: "Q1", folder: "営業", name: "集計" }],
    now: 1,
  ...
  const [c1, c2, c3] = out.payload.cards;
  assert.equal(c1.questionPath, "営業/集計", "解決できた id は再計算で更新");
  assert.equal(c2.questionPath, "相談/集計表", "未解決でも既存パスを温存");
  assert.equal(c3.questionPath, "", "未解決かつ既存パス無しは空");
});
