...
import { listCrossSearches } from "../features/analytics/crossFormSearchStore.js";
...
export default function AdminHubPage() {
  ...
  const [crossSearchesCount, setCrossSearchesCount] = useState(null);

  useCancellable(async (isCancelled) => {
    try {
      const [dashboards, questions, crossSearches] = await Promise.all([
        listDashboards(),
        listQuestions(),
        listCrossSearches(),
      ]);
      ...
      setCrossSearchesCount(crossSearches.length);
    } catch (err) {
  ...
  return (
    <AppLayout title="管理" fallbackPath="/" backHidden={false}>
      <div className="admin-hub-grid">
        ...
        <div className="main-card admin-hub-card" onClick={() => navigate("/admin/cross-searches")}>
          <h2 className="main-title">串刺しフォーム検索 管理</h2>
          <p className="nf-m-0 nf-text-muted">
            登録: {crossSearchesCount === null ? "..." : `${crossSearchesCount} 件`}
          </p>
          <div className="main-meta nf-mt-8">→ 開く</div>
        </div>
