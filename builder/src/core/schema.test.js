...
test("normalizeSchemaIDs は fileUpload 設定を allowUploadByUrl へ正規化する", () => {
  const schema = normalizeSchemaIDs([
    { type: "fileUpload", label: "添付資料", allowUploadByUrl: "true", allowFolderUrlEdit: "true", allowMultipleFiles: true, driveRootFolderUrl: "https://drive.google.com/drive/folders/abc", driveFolderNameTemplate: "{ID}" },
  ...
  assert.equal(schema[0].driveFolderNameTemplate, "{ID}");
...
test("findFirstFileUploadField はネストされた fileUpload を返す", () => {
  const fields = [
    { type: "text", label: "名前" },
    {
      type: "radio", label: "種別",
      childrenByValue: {
        "A": [{ type: "fileUpload", label: "A資料", driveFolderNameTemplate: "{ID}" }],
  ...
  assert.equal(result.driveFolderNameTemplate, "{ID}");
});
