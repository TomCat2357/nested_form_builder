...
test("normalizeSchemaIDs は fileUpload 設定を allowUploadByUrl へ正規化し、保存先フォルダ指定を撤去する", () => {
  const schema = normalizeSchemaIDs([
  ...
  // 保存先フォルダはユーザー指定不可・ID 由来固定。旧設定（allowFolderUrlEdit /
  // driveRootFolderUrl / driveFolderNameTemplate）は残っていても除去される。
  assert.equal(schema[0].allowUploadByUrl, true);
  assert.equal("allowFolderUrlEdit" in schema[0], false);
  assert.equal("driveRootFolderUrl" in schema[0], false);
...
test("normalizeExternalAction は handshakeSecret を保持しない（送信元シークレットは管理者設定へ移行）", () => {
  // 旧フォーム JSON に残る handshakeSecret は正規化で除去される。
  assert.equal("handshakeSecret" in normalizeExternalAction({ url: "https://x.com/", handshakeSecret: "s3cr3t" }), false);
});
