...
test("collectResponses: チェックボックスはフィールド 1 列にエスケープ付きカンマ連結（選択順ではなく設定順）", () => {
  const schema = [
    {
      id: "q_parent",
      type: "checkboxes",
  ...
  // view 形式: 選択肢は単一列「親」。子質問は従来どおり 親|選択肢|子 のフルパス。
  assert.deepEqual(sorted.keys, ["親", "親|B|B子", "親|A|A子"]);
  assert.deepEqual(Object.keys(sorted.map), ["親", "親|B|B子", "親|A|A子"]);
  // 選択ラベルは選択順（A,B）のまま codec で連結される。
  assert.equal(sorted.map["親"], "A,B");
  assert.equal(sorted.map["親|B|B子"], "b-value");
  assert.equal(sorted.map["親|A|A子"], "a-value");
});

test("collectResponses: ラベル内のカンマはエスケープして 1 セルに保存", () => {
  const schema = [
    {
      id: "q_c",
      type: "checkboxes",
