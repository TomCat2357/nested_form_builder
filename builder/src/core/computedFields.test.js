...
test("collectSubstitutionChildFormRefs は CHILD_FORM_* 参照から childFormId を抽出する", () => {
  const schema = [
    makeField({ id: "lk", label: "子リンク", type: "formLink", childFormId: "FORM_CHILD" }),
    makeField({ id: "s1", label: "件数", type: "substitution", templateText: "{{CHILD_FORM_COUNT(`子リンク`)}}件" }),
  ];
  const res = collectSubstitutionChildFormRefs(schema);
  assert.deepEqual(res.childFormIds, ["FORM_CHILD"]);
  assert.equal(res.hasFullQuery, false);
  assert.deepEqual(res.byFieldId.s1.childFormIds, ["FORM_CHILD"]);
  assert.equal(res.byFieldId.s1.hasFullQuery, false);
});

test("collectSubstitutionChildFormRefs は full-query を検出する（childFormId 不要）", () => {
  const schema = [
    makeField({ id: "lk", label: "子リンク", type: "formLink", childFormId: "FORM_CHILD" }),
    makeField({ id: "s1", label: "集計", type: "substitution", templateText: "{{SELECT COUNT(*) FROM [子] WHERE pid = `_id`}}" }),
  ];
  const res = collectSubstitutionChildFormRefs(schema);
  assert.equal(res.hasFullQuery, true);
  assert.equal(res.byFieldId.s1.hasFullQuery, true);
  assert.deepEqual(res.byFieldId.s1.childFormIds, []);
});

test("collectSubstitutionChildFormRefs は子フォーム非参照の置換を除外する", () => {
  const schema = [
    makeField({ id: "q1", label: "名前", type: "text" }),
    makeField({ id: "s1", label: "挨拶", type: "substitution", templateText: "{{`名前`}}さん" }),
  ];
  const res = collectSubstitutionChildFormRefs(schema);
  assert.deepEqual(res.childFormIds, []);
  assert.equal(res.hasFullQuery, false);
  assert.equal(res.byFieldId.s1, undefined);
});

// ---------------------------------------------------------------------------
// recomputeComputedFieldValues / backfillComputedFieldValues
// ---------------------------------------------------------------------------

test("recomputeComputedFieldValues は差分のある置換を上書きする", () => {
  _clearExpressionCacheForTest();
  _registerCompiledForTest("`名前`", (row) => row["名前"]);
  const schema = [
    makeField({ id: "q1", label: "名前", type: "text" }),
    makeField({ id: "s1", label: "挨拶", type: "substitution", templateText: "{{`名前`}}" }),
  ];
  const entryData = { "名前": "花子", "挨拶": "太郎" };
  const res = recomputeComputedFieldValues(schema, entryData);
  assert.equal(res.changed, true);
  assert.equal(res.data["挨拶"], "花子");
  assert.deepEqual(res.changedPaths, ["挨拶"]);
});

test("recomputeComputedFieldValues は同値なら changed=false（元参照を返す）", () => {
  _clearExpressionCacheForTest();
  _registerCompiledForTest("`名前`", (row) => row["名前"]);
  const schema = [
    makeField({ id: "q1", label: "名前", type: "text" }),
  ...
  const entryData = { "名前": "花子", "挨拶": "花子" };
  const res = recomputeComputedFieldValues(schema, entryData);
  assert.equal(res.changed, false);
  assert.equal(res.data, entryData);
});

test("recomputeComputedFieldValues は評価が空のとき既存値を据え置く", () => {
  _clearExpressionCacheForTest();
  _registerCompiledForTest("`名前`", (row) => row["名前"] || "");
  const schema = [
    makeField({ id: "q1", label: "名前", type: "text" }),
  ...
  const entryData = { "名前": "", "挨拶": "旧値" };
  const res = recomputeComputedFieldValues(schema, entryData);
  assert.equal(res.changed, false);
  assert.equal(res.data["挨拶"], "旧値");
});

test("recomputeComputedFieldValues は fieldIds で書き込み対象を限定する", () => {
  _clearExpressionCacheForTest();
  _registerCompiledForTest("`名前`", (row) => row["名前"]);
  _registerCompiledForTest("`年齢`", (row) => row["年齢"]);
  const schema = [
    makeField({ id: "q1", label: "名前", type: "text" }),
    makeField({ id: "q2", label: "年齢", type: "number" }),
    makeField({ id: "s1", label: "挨拶", type: "substitution", templateText: "{{`名前`}}" }),
    makeField({ id: "s2", label: "年齢表示", type: "substitution", templateText: "{{`年齢`}}" }),
  ];
  const entryData = { "名前": "花子", "年齢": "20", "挨拶": "旧", "年齢表示": "旧" };
  const res = recomputeComputedFieldValues(schema, entryData, undefined, ["s1"]);
  assert.equal(res.data["挨拶"], "花子");
  assert.equal(res.data["年齢表示"], "旧");
  assert.deepEqual(res.changedPaths, ["挨拶"]);
});

test("backfillComputedFieldValues は空欄のみ補完し既存値は触らない", () => {
  _clearExpressionCacheForTest();
  _registerCompiledForTest("`名前`", (row) => row["名前"]);
  const schema = [
    makeField({ id: "q1", label: "名前", type: "text" }),
  ...
  const filled = { "名前": "花子", "挨拶": "既存" };
  const r1 = backfillComputedFieldValues(schema, filled);
  assert.equal(r1.changed, false);

  const empty = { "名前": "花子" };
  const r2 = backfillComputedFieldValues(schema, empty);
  assert.equal(r2.changed, true);
  assert.equal(r2.data["挨拶"], "花子");
  assert.deepEqual(r2.newPaths, ["挨拶"]);
});
