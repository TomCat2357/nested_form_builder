...
test("traverseSchema は responses モードでも message の children を常に巡回する", () => {
  // message は値を持たないが、子質問は無条件（常に表示）なので走査対象に含める。
  const schema = [
    {
      id: "m1",
      type: "message",
      label: "お知らせ",
      children: [{ id: "mc1", type: "text", label: "補足回答" }],
    },
  ];
  const paths = [];
  traverseSchema(
    schema,
    (field, ctx) => { paths.push(ctx.pathSegments.join("|")); },
    { responses: { m1: undefined } },
  );
  assert.deepEqual(paths, ["お知らせ", "お知らせ|補足回答"]);
});
