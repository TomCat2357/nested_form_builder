...
// 保存値は素の fileId（templateId）。選択肢の value=fileId とマッピングして選択状態を復元する。
// 旧 templateUrl（URL）も resolvePrintTemplateId で吸収する。一覧に無い既存値は注記表示してデータ消失を防ぐ。
function CustomTemplateUrlFields({ printTemplateAction, updateAction, label = "カスタムテンプレートを使う" }) {
  ...
  const currentFileId = resolvePrintTemplateId(printTemplateAction);
  ...
  const handleSelect = (fileId) => {
    ...
    // 物理は素の fileId（templateId）で保持。旧 templateUrl は前進移行で落とす。
    updateAction({ templateId: opt ? opt.value : "", templateUrl: "", templatePath: opt ? (opt.path || "") : "" });
  };
        ...
        <div className="nf-col nf-gap-4">
          ...
          {hasUnlistedValue && (
            <span className="nf-text-11 nf-text-muted" style={{ wordBreak: "break-all" }}>
              現在の設定: {buildDocumentUrl(currentFileId)}（05_report_templates に無いため未選択表示。選び直すと置き換わります）
            </span>
          )}
        </div>
      )}
