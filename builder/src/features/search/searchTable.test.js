...
test("検索列は printTemplate の googleDoc 出力を「Google ドキュメント」と表示する", () => {
  const form = {
    settings: {},
    schema: [
      {
        ...
        label: "",
        isDisplayed: true,
        printTemplateAction: { enabled: true, outputType: "googleDoc", fileNameTemplate: "doc_${recordId}" },
      },
    ],
    displayFieldSettings: [
      { path: "GoogleDocument", type: "printTemplate" },
  ...
  const layout = buildSearchTableLayout(form, { includeOperations: false });
  const actionColumn = layout.columns.find((column) => column?.actionKind === "printTemplate");
  assert.equal(actionColumn?.path, "Google ドキュメント");
  const rowValues = computeRowValues({ id: "rec_1", data: {}, dataUnixMs: {} }, layout.columns);
  assert.equal(rowValues[actionColumn.key].display, "Google ドキュメント");
});
...
test("modifiedAtはJST表示を検索対象にし、raw unix文字列は部分一致検索対象にしない", () => {
  ...
  assert.equal(row.values.modifiedAt.display, "2026-01-01_09:00:00");
...
test("createdAtは表示名が作成日時でJST表示・比較検索できる", () => {
  ...
  assert.equal(row.values.createdAt.display, "2026-01-01_09:00:00");
...
test("modifiedAt比較検索は旧 YYYY/MM/DD HH:mm 形式の入力も解釈できる", () => {
  const form = { settings: {} };
  const { columns } = buildSearchTableLayout(form, { includeOperations: false });
...
test("date/time項目は canonical 文字列を YYYY-MM-DD・HH:mm:ss（既定=秒精度）で表示し検索できる", () => {
  const form = {
    settings: {},
    displayFieldSettings: [
      { path: "日付項目", type: "date" },
  ...
  // GAS が読み取り境界で date/time 列を canonical 文字列へ正規化するため、
  // フロントの entry.data には canonical 文字列が届く（Excel シリアル値の生数値は届かない）。
  const entry = {
  ...
  assert.equal(row.values[timeColumn.key].display, "14:30:00");
...
test("buildExportTableData は fileUpload 列でファイル名とフォルダハイパーリンクを出力する", () => {
  const form = { schema: [{ type: "fileUpload", label: "添付" }] };
  const entries = [
    {
      id: "rec1",
      data: {
        "添付": JSON.stringify({
          files: [
            { name: "a.pdf", driveFileId: "id1", driveFileUrl: "https://drive.google.com/file/d/id1" },
            { name: "b.docx", driveFileId: "id2", driveFileUrl: "https://drive.google.com/file/d/id2" },
          ],
  ...
  const { columns, rows } = buildExportTableData({ form, entries });
  const colIndex = columns.findIndex((c) => c?.path === "添付");
  assert.ok(colIndex >= 0, "添付列が見つからない");

  assert.deepEqual(rows[0][colIndex], {
    text: "a.pdf,b.docx",
    hyperlink: "https://drive.google.com/drive/folders/F1",
  });
  assert.deepEqual(rows[1][colIndex], {
    text: "なし",
    hyperlink: "https://drive.google.com/drive/folders/F2",
  });
  assert.equal(rows[2][colIndex], "なし");
});

// 「No.」 getValue は null と "" を区別して保持する（alasql 標準 SQL セマンティクス用）
test("createBaseColumns: No. getValue — entry に \"No.\" キー無し → sort:null", () => {
  const noCol = createBaseColumns().find((c) => c.key === "No.");
  const cell = noCol.getValue({ id: "r1" });
  assert.equal(cell.display, "");
  assert.equal(cell.search, "");
  assert.strictEqual(cell.sort, null);
});

test("createBaseColumns: No. getValue — entry[\"No.\"] === null → sort:null", () => {
  const noCol = createBaseColumns().find((c) => c.key === "No.");
  const cell = noCol.getValue({ id: "r1", "No.": null });
  assert.equal(cell.display, "");
  assert.strictEqual(cell.sort, null);
});

test("createBaseColumns: No. getValue — entry[\"No.\"] === \"\" → sort:\"\"（null とは区別）", () => {
  const noCol = createBaseColumns().find((c) => c.key === "No.");
  const cell = noCol.getValue({ id: "r1", "No.": "" });
  assert.equal(cell.display, "");
  assert.strictEqual(cell.sort, "");
});

test("createBaseColumns: No. getValue — 数値は従来通り", () => {
  const noCol = createBaseColumns().find((c) => c.key === "No.");
  const cell = noCol.getValue({ id: "r1", "No.": 5 });
  assert.equal(cell.display, "5");
  assert.equal(cell.search, "5");
  assert.strictEqual(cell.sort, 5);
});

test("createBaseColumns: No. getValue — 文字列数値は parseFloat される", () => {
  const noCol = createBaseColumns().find((c) => c.key === "No.");
  const cell = noCol.getValue({ id: "r1", "No.": "12" });
  assert.equal(cell.display, "12");
  assert.strictEqual(cell.sort, 12);
});

// 簡易検索: field="" / field:"" で空欄を抽出（数値列でも動く）
test("簡易検索: No.=\"\" は空欄行（null/\"\" 両方）にヒットし、値ありはヒットしない", () => {
  const form = { settings: {} };
  const { columns } = buildSearchTableLayout(form, { includeOperations: false });

  const entryFull = { id: "r_full", "No.": 5, modifiedAt: 0, modifiedAtUnixMs: 0, data: {}, dataUnixMs: {} };
  const entryAbsent = { id: "r_absent", modifiedAt: 0, modifiedAtUnixMs: 0, data: {}, dataUnixMs: {} }; // "No." キー無し
  const entryEmpty = { id: "r_empty", "No.": "", modifiedAt: 0, modifiedAtUnixMs: 0, data: {}, dataUnixMs: {} };

  const rowFull = { entry: entryFull, values: computeRowValues(entryFull, columns) };
  const rowAbsent = { entry: entryAbsent, values: computeRowValues(entryAbsent, columns) };
  const rowEmpty = { entry: entryEmpty, values: computeRowValues(entryEmpty, columns) };

  assert.equal(matchesKeyword(rowFull, columns, "No.=\"\""), false);
  assert.equal(matchesKeyword(rowAbsent, columns, "No.=\"\""), true);
  assert.equal(matchesKeyword(rowEmpty, columns, "No.=\"\""), true);
});

test("簡易検索: No.:\"\" / No.==\"\" は No.=\"\" と等価", () => {
  const form = { settings: {} };
  const { columns } = buildSearchTableLayout(form, { includeOperations: false });
  const entryFull = { id: "r_full", "No.": 5, modifiedAt: 0, modifiedAtUnixMs: 0, data: {}, dataUnixMs: {} };
  const entryAbsent = { id: "r_absent", modifiedAt: 0, modifiedAtUnixMs: 0, data: {}, dataUnixMs: {} };
  const rowFull = { entry: entryFull, values: computeRowValues(entryFull, columns) };
  const rowAbsent = { entry: entryAbsent, values: computeRowValues(entryAbsent, columns) };
...
test("簡易検索: No.<>\"\" / No.!=\"\" は非空欄のみヒット", () => {
  ...
  assert.equal(matchesKeyword(rowFull, columns, "No.<>\"\""), true);
  assert.equal(matchesKeyword(rowFull, columns, "No.!=\"\""), true);
  assert.equal(matchesKeyword(rowAbsent, columns, "No.<>\"\""), false);
  assert.equal(matchesKeyword(rowAbsent, columns, "No.!=\"\""), false);
});

test("簡易検索: field:null は文字列 \"null\" の部分一致（特別なセマンティクス無し）", () => {
  const form = {
    schema: [{ type: "text", label: "備考" }],
    displayFieldSettings: [{ path: "備考", type: "text" }],
    settings: {},
  };
  const { columns } = buildSearchTableLayout(form, { includeOperations: false });
  const entryNullStr = { id: "r1", "No.": 1, modifiedAt: 0, modifiedAtUnixMs: 0, data: { "備考": "this is null value" }, dataUnixMs: {} };
  const entryOther = { id: "r2", "No.": 2, modifiedAt: 0, modifiedAtUnixMs: 0, data: { "備考": "hello" }, dataUnixMs: {} };
  const entryEmpty = { id: "r3", "No.": 3, modifiedAt: 0, modifiedAtUnixMs: 0, data: { "備考": "" }, dataUnixMs: {} };

  const rowNullStr = { entry: entryNullStr, values: computeRowValues(entryNullStr, columns) };
  const rowOther = { entry: entryOther, values: computeRowValues(entryOther, columns) };
  const rowEmpty = { entry: entryEmpty, values: computeRowValues(entryEmpty, columns) };

  // "null" は単なる文字列として扱う → "null" を含む行のみヒット、空欄行はヒットしない
  assert.equal(matchesKeyword(rowNullStr, columns, "備考:null"), true);
  assert.equal(matchesKeyword(rowOther, columns, "備考:null"), false);
  assert.equal(matchesKeyword(rowEmpty, columns, "備考:null"), false);
});

test("簡易検索: No.: （引用無しの空）は ALWAYS_FALSE のまま（既存挙動維持）", () => {
  const form = { settings: {} };
  const { columns } = buildSearchTableLayout(form, { includeOperations: false });
  const entryFull = { id: "r1", "No.": 5, modifiedAt: 0, modifiedAtUnixMs: 0, data: {}, dataUnixMs: {} };
  const entryAbsent = { id: "r2", modifiedAt: 0, modifiedAtUnixMs: 0, data: {}, dataUnixMs: {} };
  const rowFull = { entry: entryFull, values: computeRowValues(entryFull, columns) };
  const rowAbsent = { entry: entryAbsent, values: computeRowValues(entryAbsent, columns) };

  // 「No.:」も「No.=」もタイポ扱い → どの行にもヒットしない
  assert.equal(matchesKeyword(rowFull, columns, "No.:"), false);
  assert.equal(matchesKeyword(rowAbsent, columns, "No.:"), false);
  assert.equal(matchesKeyword(rowFull, columns, "No.="), false);
  assert.equal(matchesKeyword(rowAbsent, columns, "No.="), false);
...
test("簡易検索: 文字列列でも =\"\" が空欄行にヒット", () => {
  ...
  const { columns } = buildSearchTableLayout(form, { includeOperations: false });
  const entryFull = { id: "r1", "No.": 1, modifiedAt: 0, modifiedAtUnixMs: 0, data: { "備考": "hello" }, dataUnixMs: {} };
  const entryEmpty = { id: "r2", "No.": 2, modifiedAt: 0, modifiedAtUnixMs: 0, data: { "備考": "" }, dataUnixMs: {} };
  const entryAbsent = { id: "r3", "No.": 3, modifiedAt: 0, modifiedAtUnixMs: 0, data: {}, dataUnixMs: {} };

  const rowFull = { entry: entryFull, values: computeRowValues(entryFull, columns) };
  const rowEmpty = { entry: entryEmpty, values: computeRowValues(entryEmpty, columns) };
  const rowAbsent = { entry: entryAbsent, values: computeRowValues(entryAbsent, columns) };

  assert.equal(matchesKeyword(rowFull, columns, "備考=\"\""), false);
  assert.equal(matchesKeyword(rowEmpty, columns, "備考=\"\""), true);
  assert.equal(matchesKeyword(rowAbsent, columns, "備考=\"\""), true);
});
