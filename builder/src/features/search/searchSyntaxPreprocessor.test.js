...
// ---------------------------------------------------------------------------
// 階層区切りは "/"（新仕様）。フィールド名に含まれる "/" はクォート / バックスラッシュでエスケープ。
// ---------------------------------------------------------------------------
test("スラッシュ区切りのネスト列名（識別子は __ に正規化される）", () => {
  const r = preprocessSearchQuery("親質問/子質問 = '値'", COLS);
  assert.equal(r.expr, "`親質問__子質問` = '値'");
});

test("バックスラッシュエスケープされた / は列名の一部（aaa/bbb/cc\\/c）", () => {
  const r = preprocessSearchQuery("aaa/bbb/cc\\/c = '値'", COLS);
  assert.equal(r.expr, "`aaa__bbb__cc/c` = '値'");
});

test("クォートで囲んだセグメントの / は列名の一部（aaa/bbb/'cc/c'）", () => {
  const r = preprocessSearchQuery("aaa/bbb/'cc/c' = '値'", COLS);
  assert.equal(r.expr, "`aaa__bbb__cc/c` = '値'");
});

test("ダブルクォートで囲んだセグメントの / も列名の一部", () => {
  const r = preprocessSearchQuery('aaa/bbb/"cc/c" = \'値\'', COLS);
  assert.equal(r.expr, "`aaa__bbb__cc/c` = '値'");
});

test("バックティック内のスラッシュ区切りも __ に正規化", () => {
  const r = preprocessSearchQuery("`親質問/子質問` = '値'", COLS);
  assert.equal(r.expr, "`親質問__子質問` = '値'");
});

test("値側のクォート文字列に含まれる / はパス区切りにならない", () => {
