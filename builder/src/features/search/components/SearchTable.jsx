...
const renderHitSegments = (segments) =>
  segments.map((seg, i) =>
    seg.hit ? <strong key={i}>{seg.text}</strong> : <span key={i}>{seg.text}</span>,
  );

const renderHitExcerpts = (hits) => {
  if (!Array.isArray(hits) || hits.length === 0) {
    return <span className="nf-text-muted">(他の項目に一致)</span>;
  }
  return hits.map((hit) => (
    <div key={hit.columnKey} className="search-hit-line">
      <span className="nf-text-muted">{hit.columnLabel}：</span>
      {renderHitSegments(hit.segments)}
    </div>
  ));
};
            ...
            {headerRows.map((headerRow, rowIndex) => (
              <tr key={`header-row-${rowIndex}`}>
                ...
                {headerRow.map((cell, cellIndex) => {
                  ...
                  const isHitColumn = column?.key === HIT_EXCERPT_COLUMN_KEY;

                  return (
                    <th
                      key={`header-cell-${rowIndex}-${cellIndex}`}
                      className={`search-th${isHitColumn ? " search-th-hit" : ""}`}
                      data-sortable={sortable && cell.label !== "" ? "true" : "false"}
                      colSpan={cell.colSpan}
                      rowSpan={cell.rowSpan ?? 1}
                      style={isHitColumn && hitColumnMinWidth ? { minWidth: `${hitColumnMinWidth}px` } : undefined}
              ...
              return (
                ...
                >
                  ...
                  {selectableColumns.map((column) => {
                    if (column.key === HIT_EXCERPT_COLUMN_KEY) {
                      return (
                        <td
                          key={`${entry.id}_${column.key}`}
                          className="search-td search-td-hit"
                          style={hitColumnMinWidth ? { minWidth: `${hitColumnMinWidth}px` } : undefined}
                        >
                          {renderHitExcerpts(row.hitExcerpts)}
                        </td>
                      );
                    }
