...
export const createBaseColumns = () => [
  ...
  },
  {
    ...
    getValue: (entry) => {
      const raw = entry?.["No."];
      // entry に "No." キーが無い / 値が null → 真の NULL（alasql 行にも null として届く）
      if (raw === null || raw === undefined) {
        return { display: "", search: "", sort: null };
      }
      // 値が "" → 空文字として保持（null とは区別）
      if (raw === "") {
      ...
      return {
        ...
        sort: typeof raw === "number" ? raw : (parseFloat(display) || 0),
      };
    },
  },
  {
    ...
    searchable: true,
    getValue: (entry) => {
      // entry.createdAt は JST 文字列 `YYYY/MM/DD HH:mm:ss.SSS`。
      // 文字列ならそのまま canonical なので辞書順 = 時系列順でソートできる。
      // 旧データ救済として Unix ms シム / 数値も受け付ける。
      const raw = entry?.createdAt ?? "";
      const isCanonicalJstString = typeof raw === "string" && raw && /^\d{4}-\d{2}-\d{2}/.test(raw);
      const unixMs = toUnixMs(entry?.createdAtUnixMs ?? raw);
      const display = Number.isFinite(unixMs)
        ? formatUnixMsDateTimeSec(unixMs)
        : (typeof raw === "string" ? raw : "");
      return {
        display,
        search: normalizeSearchText(display || ""),
        sort: isCanonicalJstString
          ? raw
          : (Number.isFinite(unixMs) ? unixMs : ""),
      };
    },
  },
  {
...
export const buildSearchColumns = (form, { includeOperations = true } = {}) => {
  ...
  resolveDisplayFieldSettings(form).forEach(({ path, type, optionOrder, fieldId, printTemplateAction: resolvedAction }) => {
    ...
    parentColumns.push(createDisplayColumn(path, type, {
      optionOrder,
      fieldId,
      timePrecision: type === "time" ? (fieldMeta?.timePrecision || "second") : "",
    }));
  });
...
// 検索ヒット箇所列（表示専用）。簡易検索時に最左へ挿入する。
// segments 長 1 なので階層ヘッダーがあっても最上行のみにラベルが出る。
export const HIT_EXCERPT_COLUMN_KEY = "__hit";

// 検索ヒット箇所列の最小幅（px）の既定値。
// テーブルは table-layout: auto で、ヒット列以外は white-space: nowrap のため
// 折り返せるヒット列が常にしわ寄せ役になって細くなる。最小幅を与えて防ぐ。
// 設定（searchHitColumnMinWidth）で上書き可能。
export const DEFAULT_HIT_COLUMN_MIN_WIDTH = 280;

export const createHitExcerptColumn = () => ({
  key: HIT_EXCERPT_COLUMN_KEY,
  segments: ["検索ヒット箇所"],
  sortable: false,
  searchable: false,
  getValue: () => ({ display: "", search: "", sort: "" }),
...
export const buildHeaderRowsLayout = (columns) => {
  ...
  return deduped.map((row, rowIndex) =>
...
export const buildSearchTableLayout = (
  ...
  const headerRows = buildHeaderRowsLayout(columns);
