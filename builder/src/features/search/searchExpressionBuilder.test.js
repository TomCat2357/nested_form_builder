...
test("buildSearchExpression: 表示列に対する = は識別子が path ベースに解決される", () => {
  const r = buildSearchExpression("氏名 = '田中'", COLUMNS);
  assert.equal(r.expr, "`氏名` = '田中'");
  assert.deepEqual(r.errors, []);
});

test("buildSearchExpression: 日付型列 = 日付リテラルは列を丸めずリテラルのみ canonical 化（生文字列比較）", () => {
  const r = buildSearchExpression("販売日 = 2026/04/01", COLUMNS);
  assert.equal(r.expr, "`販売日` = '2026/04/01'");
});

test("buildSearchExpression: 日付型列 > 日付リテラルもリテラルのみ canonical 化（区切りゆらぎを吸収）", () => {
  const r = buildSearchExpression("販売日 > 2020-4-1", COLUMNS);
  assert.equal(r.expr, "`販売日` > '2020/04/01'");
});

test("buildSearchExpression: createdAt は検索対象（datetime メタ列。リテラルのみ canonical 化）", () => {
  // createdAt は metaByName に含まれ isDateLike 判定が効くので、日付リテラルの区切り/ゼロ埋めゆらぎを吸収する。
  const r = buildSearchExpression("createdAt >= 2026/1/1", COLUMNS);
  assert.equal(r.expr, "`createdAt` >= '2026/01/01'");
});

test("buildSearchExpression: modifiedAt は検索対象（列は丸めずリテラルのみ canonical 化）", () => {
  const r = buildSearchExpression("modifiedAt >= 2026/01/01", COLUMNS);
...
test("buildSearchExpression: パイプ列名は __ に正規化される", () => {
  const r = buildSearchExpression("親質問|子質問 = '値'", COLUMNS);
  assert.equal(r.expr, "`親質問__子質問` = '値'");
});

test("buildSearchExpression: 数値列の比較は素通し", () => {
  const r = buildSearchExpression("No. >= 10", COLUMNS);
...
test("buildSearchExpression: 日付列は列を丸めず文字列比較（view/data 共通）", () => {
  // view 行も data 行も date 列を canonical 文字列 (YYYY/MM/DD) で持つので、固定長辞書順 = 時系列順。
  const r = buildSearchExpression("販売日 >= 2020/04/01", COLUMNS);
  assert.equal(r.expr, "`販売日` >= '2020/04/01'");
  assert.deepEqual(r.errors, []);
});

test("buildSearchExpression: modifiedAt も文字列比較", () => {
  const r = buildSearchExpression("modifiedAt >= 2026/01/01", COLUMNS);
  assert.equal(r.expr, "`modifiedAt` >= '2026/01/01'");
});
