...
test("buildPrintDocumentPayload は印刷用 driveSettings に ID と仮フォルダ情報を含める", () => {
  const payload = buildPrintDocumentPayload({
    schema: [
      { id: "name", type: "text", label: "氏名" },
      { id: "upload", type: "fileUpload", label: "添付" },
    ],
    responses: {
      name: "山田 太郎",
    },
...
test("buildRecordItems は fileUpload にファイル URL（driveFileId 再構成）とフォルダ URL を添える", () => {
  ...
  const items = buildRecordItems(schema, responses, {
    folderUrlsByField: { f1: "https://drive.google.com/drive/folders/FOLDER1" },
    folderNamesByField: { f1: "06_upload_files/r1" },
  });
  assert.equal(items.length, 1);
  const item = items[0];
  assert.equal(item.type, "fileUpload");
  assert.equal(item.folderUrl, "https://drive.google.com/drive/folders/FOLDER1");
  assert.equal(item.folderName, "06_upload_files/r1");
  assert.deepEqual(item.files, [
    { name: "見積.pdf", url: "https://drive.google.com/file/d/FID1/view", driveFileId: "FID1" },
    { name: "図面.pdf", url: "https://drive.google.com/file/d/FID2/view" },
  ]);
});

test("buildRecordItems は添付なし/フォルダなしの fileUpload に files/folderUrl を付けない", () => {
  const schema = [{ id: "f1", type: "fileUpload", label: "添付" }];
