...
test("buildDriveUploadSettings は folderState から driveSettings を構築し、保存先フォルダ指定は持ち込まない", () => {
  const settings = buildDriveUploadSettings({
    ...
    field: {
      // 旧フォームに残っていてもアップロード設定としては無視する（常に 06_upload_files 直下）。
  ...
  // 保存先ルート・フォルダ名テンプレはユーザー指定不可なので driveSettings に乗らない。
  assert.equal("rootFolderUrl" in settings, false);
  assert.equal("folderNameTemplate" in settings, false);
  assert.equal(settings.folderUrl, "https://drive.google.com/drive/folders/folder");
  assert.equal(settings.autoCreated, true);
});

test("buildDriveUploadSettings は field/driveSettings が欠けていても folderUrl/autoCreated を埋める", () => {
  const settings = buildDriveUploadSettings({
    folderState: baseState(),
    field: undefined,
...
test("validateUploadFile は通常ファイルを許可する（空文字を返す）", () => {
  assert.equal(validateUploadFile({ name: "photo.png", size: 1024 }), "");
  assert.equal(validateUploadFile({ name: "report.PDF", size: 5 * 1024 * 1024 }), "");
  assert.equal(validateUploadFile({ name: "no-extension", size: 100 }), "");
});

test("validateUploadFile は 25MB 超を拒否する", () => {
  assert.equal(
    validateUploadFile({ name: "big.zip", size: 25 * 1024 * 1024 + 1 }),
    "ファイルサイズが上限(25MB)を超えています",
  );
  // ちょうど 25MB は許可
  assert.equal(validateUploadFile({ name: "edge.zip", size: 25 * 1024 * 1024 }), "");
});

test("validateUploadFile は危険拡張子を大文字小文字問わず拒否する", () => {
  assert.equal(validateUploadFile({ name: "malware.exe", size: 10 }), "このファイル形式はアップロードできません: .exe");
  assert.equal(validateUploadFile({ name: "script.JS", size: 10 }), "このファイル形式はアップロードできません: .js");
  assert.equal(validateUploadFile({ name: "run.BAT", size: 10 }), "このファイル形式はアップロードできません: .bat");
});

test("validateUploadFile は file が無いと空文字を返す", () => {
  assert.equal(validateUploadFile(null), "");
  assert.equal(validateUploadFile(undefined), "");
});
