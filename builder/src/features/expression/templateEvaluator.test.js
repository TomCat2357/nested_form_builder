...
// ---------------------------------------------------------------------------
// 二重ブレース {{ ... }} は viewRow、単一 { ... } は data row で評価する
// ---------------------------------------------------------------------------

test("resolveTemplate: {} は data row、{{}} は viewRow を引く", () => {
  setup();
  _registerCompiledForTest("`性別`", (row) => row["性別"]);
  const dataRow = { 性別: true };
  const viewRow = { 性別: "男" };
  // 単一ブレース → data row（真偽値）
  assert.equal(resolveTemplate("{`性別`}", dataRow, { viewRow }), "true");
  // 二重ブレース → view row（ラベル）
  assert.equal(resolveTemplate("{{`性別`}}", dataRow, { viewRow }), "男");
});

test("resolveTemplate: data と view を 1 テンプレートに混在", () => {
  setup();
  _registerCompiledForTest("`性別__男`", (row) => row["性別__男"]);
  _registerCompiledForTest("`性別`", (row) => row["性別"]);
  const dataRow = { "性別__男": true };
  const viewRow = { 性別: "男" };
  const out = resolveTemplate("元:{`性別|男`}/ビュー:{{`性別`}}", dataRow, { viewRow });
  assert.equal(out, "元:true/ビュー:男");
});

test("resolveTemplate: viewRow 未指定なら {{}} も data row を使う", () => {
  setup();
  _registerCompiledForTest("`x`", (row) => row["x"]);
