...
test("UNIQUE_CSV: 各要素の前後空白を除去（内部空白は保持）", () => {
  const alasql = makeFakeAlaSql();
  ensureNfbUdfsRegistered(alasql);
  const fn = alasql.fn.UNIQUE_CSV;
  // 前後空白除去
  ...
  // 空白のみのセルは trim 後に空 → 除外。値の先頭空白も除去して重複判定。
  assert.equal(fn(", はこわな, , , 空気銃, , ライフル銃"), "はこわな,空気銃,ライフル銃");
  // trim 後に同一になる要素は重複扱いで初出のみ残す
  assert.equal(fn("a, a , b"), "a,b");
});
