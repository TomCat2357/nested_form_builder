...
test("CHART_AXIS_REQUIREMENTS は Phase1 拡張をカバー", () => {
  for (const t of ["stackedBar", "row", "area", "combo", "donut"]) {
    assert.ok(CHART_AXIS_REQUIREMENTS[t], "missing: " + t);
  }
});

test("CHART_AXIS_REQUIREMENTS は Phase2 拡張をカバー", () => {
  for (const t of ["number", "trend", "progressBar", "gauge", "detail", "pivotTable", "waterfall", "funnel"]) {
    assert.ok(CHART_AXIS_REQUIREMENTS[t], "missing: " + t);
  }
});

test("CHART_AXIS_REQUIREMENTS は Phase3/4 拡張をカバー", () => {
  for (const t of ["sunburst", "sankey", "pinMap", "gridMap", "regionMap"]) {
    assert.ok(CHART_AXIS_REQUIREMENTS[t], "missing: " + t);
  }
});
