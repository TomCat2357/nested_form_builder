...
test("count(*) のみ・グループ化なし", () => {
  ...
  assert.deepEqual(r.columns, [{ name: "a_1", role: "metric", aggId: "a_1", aggType: "count" }]);
});
...
test("columns 配列は dimension → metric の順で metric には aggId が付く", () => {
  ...
  assert.deepEqual(r.columns, [
    { name: "基本情報__区", role: "dimension" },
    { name: "a_1", role: "metric", aggId: "a_1", aggType: "count" },
    { name: "a_2", role: "metric", aggId: "a_2", aggType: "sum" },
  ]);
});
