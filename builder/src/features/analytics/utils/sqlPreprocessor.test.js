...
// formLink 子フォームを許可集合に含めると、置換 full-query から `FROM [子]` ＋ `pid` 結合を
// 参照できる（runFullQuery が {defaultFormId} ∪ {formLink childFormId} を渡すケース）。
test("allowedFormIds に子フォーム id を含めると FROM [子] ＋ pid 結合が解決される", () => {
  const r = preprocessSql("SELECT [備考] FROM [別フォーム] WHERE [pid] = 'p1'", {
    defaultFormId: formA.id, formIndex, getColumnIndex,
    allowedFormIds: new Set([formA.id, formB.id]),
  });
  assert.equal(r.ok, true, r.errors.join(" / "));
  assert.equal(r.referencedFormIds.includes(formB.id), true);
  // pid はメタ列なので素通し（[...] のまま残る）。
