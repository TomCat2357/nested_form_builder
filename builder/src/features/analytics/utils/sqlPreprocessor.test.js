...
test("検索 SQL モード: FROM _ は自フォーム（defaultFormId）の canonical を AS _ で貼る", () => {
...
test("検索 SQL モード: _.[col] は _ alias 修飾で解決される", () => {
...
test("検索 SQL モード: defaultFormId 未指定で FROM _ はエラー", () => {
  const r = preprocessSql("SELECT * FROM _", {
    defaultFormId: null, formIndex, getColumnIndex,
  });
