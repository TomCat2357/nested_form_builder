...
test("resolveFormRef はフォルダ込み名を | 区切りでも解決する（/ と等価）", () => {
  const idx = buildFormIndex([
    mkFormF("fa", "苦情データ", "受付", 1),
    mkFormF("fb", "苦情データ", "営業/見積", 2),
  ]);
  assert.equal(resolveFormRef("受付|苦情データ", idx).id, "fa");
  assert.equal(resolveFormRef("営業|見積|苦情データ", idx).id, "fb");
  // / と | の混在も同じフォームに解決する
  assert.equal(resolveFormRef("営業/見積|苦情データ", idx).id, "fb");
  // | を含むトークンはバレ名として曖昧扱いしない
  assert.equal(isAmbiguousBareTitle("受付|苦情データ", idx), false);
});
