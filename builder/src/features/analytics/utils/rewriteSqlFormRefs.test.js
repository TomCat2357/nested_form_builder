...
// ---------------------------------------------------------------------------
// remapFormRefIds（読取時 case ①: 死んだ fileId → 現 fileId へ実行用に貼り替え）
// ---------------------------------------------------------------------------

test("remapFormRefIds: FROM/JOIN/[ref].[col] の旧 fileId を新 fileId へ置換", () => {
  const remap = { dead1: "f_complaint", dead2: "f_other" };
  const out = remapFormRefIds(
    "SELECT [dead1].[区] FROM [dead1] AS a JOIN [dead2] AS b ON 1=1",
    remap,
  );
  assert.equal(out, "SELECT [f_complaint].[区] FROM [f_complaint] AS a JOIN [f_other] AS b ON 1=1");
});

test("remapFormRefIds: remap に無い参照・data・リテラルは素通し", () => {
  const out = remapFormRefIds(
    "SELECT '[dead1]' AS lit FROM [data] JOIN [alive] ON 1=1",
    { dead1: "f_complaint" },
  );
  assert.equal(out, "SELECT '[dead1]' AS lit FROM [data] JOIN [alive] ON 1=1");
});

test("remapFormRefIds: remap 空 / null / 空SQL は素通し", () => {
  assert.equal(remapFormRefIds("SELECT * FROM [dead1]", {}), "SELECT * FROM [dead1]");
  assert.equal(remapFormRefIds("SELECT * FROM [dead1]", null), "SELECT * FROM [dead1]");
  assert.equal(remapFormRefIds("", { a: "b" }), "");
});

// ---------------------------------------------------------------------------
// recoverDeadFormRefs（読取時 case ①: 死んだ formId を formPath で引き直して SQL/formSources を復旧）
// ---------------------------------------------------------------------------

test("recoverDeadFormRefs: 死んだ formId を formPath で現 fileId へ復旧し SQL を貼り替える", () => {
  // 保存時は formId=DEAD（旧 id）で SQL も [DEAD] だが、現在その物理は無く、論理パス 相談/対応一覧 が
  // 現フォーム f_in_folder を指す。formPath で引き直して [DEAD]→[f_in_folder] に貼り替わること。
  const sources = [{ formId: "DEAD", formPath: "相談/対応一覧", alias: "data" }];
  const out = recoverDeadFormRefs("SELECT [DEAD].[氏名] FROM [DEAD]", sources, formIndex);
  assert.deepEqual(out.remap, { DEAD: "f_in_folder" });
  assert.equal(out.sql, "SELECT [f_in_folder].[氏名] FROM [f_in_folder]");
  assert.deepEqual(out.formSources, [{ formId: "f_in_folder", formPath: "相談/対応一覧", alias: "data" }]);
});

test("recoverDeadFormRefs: formId が生存していれば触らない（remap 空・SQL/sources 不変）", () => {
  const sources = [{ formId: "f_complaint", formPath: "苦情データ" }];
  const out = recoverDeadFormRefs("SELECT * FROM [f_complaint]", sources, formIndex);
  assert.deepEqual(out.remap, {});
  assert.equal(out.sql, "SELECT * FROM [f_complaint]");
  assert.deepEqual(out.formSources, sources);
});

test("recoverDeadFormRefs: formPath 無し / 解決不能なら据え置き（remap 空）", () => {
  const noPath = recoverDeadFormRefs("SELECT * FROM [DEAD]", [{ formId: "DEAD" }], formIndex);
  assert.deepEqual(noPath.remap, {});
  assert.equal(noPath.sql, "SELECT * FROM [DEAD]");
  const badPath = recoverDeadFormRefs("SELECT * FROM [DEAD]", [{ formId: "DEAD", formPath: "存在しない/フォーム" }], formIndex);
  assert.deepEqual(badPath.remap, {});
});
