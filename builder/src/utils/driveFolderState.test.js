...
test("markDriveFolderForDeletion は永続(KEEP)フォルダを消さず no-op で返す", () => {
  const keep = {
    resolvedUrl: "https://drive.google.com/drive/folders/keep123",
    inputUrl: "https://drive.google.com/drive/folders/keep123",
    folderName: "NFB_RECORD_KEEP_r_abc_12345678",
    autoCreated: true,
  };
  const result = markDriveFolderForDeletion(keep);
  // 保存先・論理パスを保持し、pendingDeleteUrl は付けない（保護）。
  assert.equal(result.resolvedUrl, "https://drive.google.com/drive/folders/keep123");
  assert.equal(result.folderName, "NFB_RECORD_KEEP_r_abc_12345678");
  assert.equal(result.pendingDeleteUrl, "");
});
