...
test("resolveTemplateTokens: full-query を囲む式トークン（UNIQUE_CSV(...)）も queryTokenValues から解決", () => {
  setup();
  // 外側は full-query ではないが内側に {{SELECT}} を含む。prefetch が外側トークンの
  // fullToken をキーに最終値を載せる想定を再現し、同期解決が Map 値を引くことを確認する。
  const tpl = "手段: {{UNIQUE_CSV({{SELECT [m] FROM [子] WHERE [pid]=_id}})}}";
  const outer = collectBalancedBraces(escapeBraces(tpl))[0];
  const queryTokenValues = new Map([[outer.fullToken, "はこわな,空気銃,ライフル銃"]]);
  assert.equal(
    resolveTemplateTokens(tpl, { queryTokenValues }),
    "手段: はこわな,空気銃,ライフル銃",
  );
});
...
test("injectResolvedQueryTokens: full-query を囲む式トークンも解決値へ差し替え", () => {
  const tpl = "{{`氏名`}}-手段:{{UNIQUE_CSV({{SELECT [m] FROM [子]}})}}";
  const tokens = collectBalancedBraces(escapeBraces(tpl));
  // 外側 UNIQUE_CSV(...) トークンを prefetch 済み値に。
  const wrap = tokens.find((t) => t.body.includes("UNIQUE_CSV"));
  const queryTokenValues = new Map([[wrap.fullToken, "はこわな,空気銃"]]);
  const out = injectResolvedQueryTokens(tpl, queryTokenValues);
  // 囲みトークンはフロントで完全解決され、純粋な式トークンは GAS 用に原文のまま。
  assert.equal(out, "{{`氏名`}}-手段:はこわな,空気銃");
});
