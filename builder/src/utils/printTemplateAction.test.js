...
test("usesPrintTemplatePdfLink は Gmail 本文の {_PDF} 利用有無を判定する", () => {
  ...
  assert.equal(usesPrintTemplatePdfLink({ outputType: "gmail", gmailTemplateBody: "本文 {_DOCUMENT}" }), false);
});
...
test("resolveEffectivePrintTemplateFileNameTemplate は Gmail の {_PDF} でフォーム共通か既定値のみを使う", () => {
  assert.equal(
    resolveEffectivePrintTemplateFileNameTemplate(
