...
test("restoreResponsesFromData: view 形式（フィールド 1 列）を選択ラベルへ復元する", () => {
  const data = {
    チェック項目: "A,B",
    ラジオ項目: "低",
    選択項目: "Y",
...
test("restoreResponsesFromData: チェックボックスは配列形式（インメモリ）も復元する", () => {
  const data = {
    チェック項目: ["C", "A"],
    ラジオ項目: "高",
    選択項目: "X",
...
test("restoreResponsesFromData: options外のラベルも復元する（codec 分割・末尾に追加）", () => {
  const data = {
    チェック項目: "その他",
    選択項目: "外部",
  };
...
test("restoreResponsesFromData: ラベル内カンマ（エスケープ）を正しく分解して復元する", () => {
  const localSchema = [{
    id: "f_c",
    type: "checkboxes",
    label: "色",
    options: [{ label: "赤, 青" }, { label: "カラス" }],
  }];
  const restored = restoreResponsesFromData(localSchema, { 色: "赤\\, 青,カラス" }, {});
  assert.deepEqual(restored.f_c, ["赤, 青", "カラス"]);
});

test("restoreResponsesFromData: 入力フィールドの children は親に値があるときのみ復元する", () => {
  const childSchema = [
    {
      id: "f_parent",
      type: "text",
      label: "親",
      children: [
        { id: "f_child", type: "text", label: "子" },
      ],
    },
  ];

  const restored = restoreResponsesFromData(childSchema, { 親: "値あり", "親|子": "子の値" }, {});
  assert.equal(restored.f_parent, "値あり");
  assert.equal(restored.f_child, "子の値");

  const restoredEmpty = restoreResponsesFromData(childSchema, { 親: "", "親|子": "孤児" }, {});
  assert.equal(restoredEmpty.f_parent, "");
  assert.equal(restoredEmpty.f_child, undefined, "親が空のとき子は復元しない");
});
