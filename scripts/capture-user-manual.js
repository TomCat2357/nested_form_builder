...
async function buildManualForm(frame) {
  ...
  // 初期値・入力制限はセレクトボックス（option の value で対象セレクトを特定する）
  await q1.locator('select:has(option[value="userName"])').first().selectOption("userName");
  await q1.locator('select:has(option[value="pattern"])').first().selectOption("pattern");
  ...
  // 様式出力（printTemplate）の質問をフォーム末尾に追加する。
  // 末尾追加には EditorPage 直下の「質問を追加」ボタンを使う（カード内の「次の質問を追加」だと
  // 直前カードに子質問がある場合に挿入位置がずれるため）。
  await frame.getByRole("button", { name: "質問を追加", exact: true }).click({ force: true });
  await frame.page().waitForTimeout(700);

  const q6 = frame.locator('[data-question-id][data-depth="0"]').last();
  await q6.getByPlaceholder("項目名を入力").fill("申請書を出力");
  await q6.locator("select").first().selectOption("printTemplate");
  await frame.page().waitForTimeout(700);
...
async function captureFormsAndImport(context) {
  ...
  try {
    ...
    await frame.getByRole("button", { name: /インポート/ }).first().click();
    const importInput = frame.locator('input[placeholder^="https://drive.google.com/file/d/"]').first();
    await importInput.waitFor({ state: "visible", timeout: 30000 });
    await importInput.fill("https://drive.google.com/file/d/FILE_ID/view");
    await frame.page().waitForTimeout(500);
    await saveScreenshot(iframe, "manual_05_import_dialog.png");
  } finally {
    await page.close();
  }
}
...
async function captureEditorScreens(context, formId) {
  ...
  try {
    ...
    const q6 = await getTopLevelCardByValue(frame, "申請書を出力");
    ...
    // 様式出力カード（PDF / Google ドキュメントモード）
    await q6.scrollIntoViewIfNeeded();
    await saveScreenshot(q6, "manual_15_print_template_card_pdf.png");

    // 様式出力カード（Gmail モード）
    await q6.locator('select:has(option[value="gmail"])').first().selectOption("gmail");
    await frame.page().waitForTimeout(700);
    await setCheckboxByLabel(q6, "PDF を添付", true);
    ...
    await frame.page().waitForTimeout(500);
    await q6.scrollIntoViewIfNeeded();
    await saveScreenshot(q6, "manual_16_print_template_card_gmail.png");
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.warn("search/form-input のキャプチャをスキップしました:", error.message);
    }
    const { page, frame } = await openApp(context);
    try {
      await cleanupSampleForms(frame);
