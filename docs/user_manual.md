...
fileUpload 質問が 1 つ以上あるフォームでは、レコードごとに **「ルートフォルダの中に、レコード専用の子フォルダ」** を作って添付ファイルを集約します。このフォルダ URL はレコードの `driveFolderUrl` 列にも保存され、`{@_folder_url}` トークンで様式出力・Gmail 下書きから参照できます。

fileUpload 質問カードで設定するのは次の 3 段階です。

1. **ルートフォルダURL**: そのフォームで扱うファイルの親フォルダ。空白ならマイドライブ直下に作成
2. **フォルダ命名規則**: レコードごとの子フォルダ名テンプレート。空白なら子フォルダを作らずルート直下に置く
3. **アップロード動作の微調整**: URL 入力の許可・ユーザーによる保存先変更の許可・拡張子の非表示

同じフォームに複数の fileUpload 質問があっても、**レコードのフォルダは 1 つ**にまとめられます。どの fileUpload カードでアップロードしても、同じレコード用フォルダが再利用されます。
...
`{@_file_urls}` トークンは、そのレコードに紐づく**全 fileUpload 質問**のファイル URL を `, ` 区切りで返します（印刷様式・Gmail 本文で使用可）。`{@_folder_url}` はレコード専用フォルダそのものの URL（**Gmail 本文と `if:` 条件でのみ有効**）。

```text
■ 添付フォルダ: {@_folder_url|default:（未作成）}
■ 添付ファイル: {@_file_urls|if:@_file_urls,なし}
```
...
| 件名 | 件名テンプレート |
| 本文 | 本文テンプレート。`{@_folder_url}`, `{@_record_url}`, `{@_form_url}`, `{@_file_urls}` といった予約トークンも使えます |

Gmail 専用の予約トークン（`_folder_url`, `_record_url`, `_form_url`）は、**Gmail 本文以外では空文字**になります。ただし `if:` / `ifv:` / `default:` の条件や真偽判定では PDF モードでも値として評価できます。
...
■ 添付フォルダ: {@_folder_url|default:（未作成）}
■ 添付ファイル: {@_file_urls|ifv:@_file_urls,あり,なし}
...
添付: {@添付書類|ifv:@_file_urls,添付あり,添付なし}
```
...
| `{@_folder_url}` | レコードの Drive フォルダ URL（**Gmail 本文と `if:`/`ifv:`/`default:` でのみ有効**） |
| `{@_record_url}` | レコード閲覧 URL（Gmail 本文と条件判定でのみ） |
| `{@_form_url}` | フォーム入力 URL（Gmail 本文と条件判定でのみ） |
| `{@_file_urls}` | レコード内の全 fileUpload 質問から集めた URL（カンマ区切り） |
...
- `@フィールド名` — 他フィールドの値
- `@_予約トークン` — `@_folder_url` など予約トークン（条件内なら PDF モードでも参照可）
- `"リテラル"` — ダブルクォート囲みの文字列
- `数字` — 数値として比較
...
{@_file_urls|ifv:@_file_urls,添付あり,添付なし}
    → 添付ファイルの有無で表記を切り替え

{@備考|if:@_folder_url,（補足はフォルダ参照）}
...
fileUpload の値は URL 文字列（未アップロード時は空）です。`default` や `ifv` で出し分けできます。

```text
{@添付ファイル|default:なし}
    → URL があれば URL、なければ「なし」
...
### Q2. Gmail のテンプレートで `{@_folder_url}` を書いても空になる
...
**1 レコードにつき 1 フォルダ**です。どの fileUpload 質問でも同じレコードフォルダに保存されます。`ルートフォルダURL` と `フォルダ命名規則` はすべての fileUpload 質問で整合するように設定してください（違うルートを指定しても、レコードの 1 つ目のフォルダに集約されます）。
...
fileUpload 質問の `ルートフォルダURL` が他の質問と食い違っていないか確認。同じレコード内では最初のアップロードでフォルダが確定し、以降は同じフォルダが使われます。
