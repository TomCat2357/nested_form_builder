...
- 各フォームカードには**フォーム名**、**説明**、**最終更新日時**が表示されます
...
GAS側の設定は Google Apps Script の **PropertiesService** を使って管理されており、設定の種類によって保存される**スコープ（有効範囲）が異なります**。

| 設定項目 | プロパティスコープ | 有効範囲 |
|----------|-------------------|----------|
| **管理者キー** | Script Properties（スクリプトプロパティ） | スクリプト全体で共有。全ユーザー共通 |
| **ユーザー設定**（スプレッドシートID・シート名・GAS URL等） | User Properties（ユーザープロパティ） | アクセスしたGoogleアカウントごとに独立 |

**Script Properties（スクリプトプロパティ）の特徴**:
- 同じGASスクリプトを利用する全ユーザーで共有される
- 管理者キーの変更は即座に全員に反映される
- GASスクリプトエディタの「プロジェクトの設定」→「スクリプトプロパティ」から手動確認・編集が可能

**User Properties（ユーザープロパティ）の特徴**:
- GAS Webアプリへのアクセス時のアクティブなGoogleアカウントに紐づく
- アカウントが異なると、それぞれ独立した設定が適用される
- 同じデプロイを複数人で共用する場合、ユーザーごとに異なるスプレッドシートを指定できる

> **注意**: 管理者キーの変更はScript Propertiesに即座に反映されます。一方、ユーザー設定はアカウントごとに独立しているため、ブラウザでアクセスするGoogleアカウントを切り替えると、保存済みの設定値が異なる場合があります。
...
**最終更新**: 2026年2月18日（v2.5: サービスプロパティスコープの説明を追加）
