...
`Question 作成` を押すと作成画面に入ります。流れは次の 3 ステップです。

1. **Question 名** を入力（例: `日別件数`）
2. **データソース** で集計対象のフォームを選ぶ → 列情報の取得を待つ
3. **集計**・**グループ化**・**フィルター**・**上限行数** を組み立てて `クエリ実行`

`GUI` モードでは集計種別（件数 / 非空件数 / 合計 / 平均 / 最小 / 最大 / 集計なし[生データ]）と、複数のグループ化列、フィルター条件をフォームから設定できます。SQL を直接書きたい場合は上部の `SQL` タブに切り替えます。

クエリ実行に成功すると `可視化` セクションが下部に出現し、グラフ種別を選べます。

| グラフ種別 | 想定用途 |
| --- | --- |
| テーブル | 結果をそのまま表で確認 |
| 単一値 | 1 行 1 列のスカラ値（COUNT 結果など） |
| 棒グラフ | カテゴリ別の件数・金額比較 |
| 折れ線グラフ | 時系列の推移（日付ビン推奨） |
| 円グラフ | 構成比 |
| 散布図 | 数値 2 軸の相関（生データモードで使用） |

X 軸 / Y 軸の列は出力列名から選択でき、Y 軸はカンマ区切りで複数指定できます。

### 9.2 日付列で集計する

日付型の列をグループ化に追加すると、ビン（粒度）として `そのまま` / `年単位` / `月単位` / `日単位` を選べます。`日単位` を選ぶと X 軸ラベルが `YYYY-MM-DD`、`月単位` なら `YYYY-MM` の形式になります。`そのまま` は元の値を素のまま使うため、エポック ms で保持されているフォームでは数値ラベルが表示されます。

![日単位ビンで件数を棒グラフ化](user_manual_images/manual_20_question_bar_day.png)

`月単位` に切り替えると、同月のレコードが 1 本のバーにまとまります。

![月単位ビンで件数を棒グラフ化](user_manual_images/manual_21_question_bar_month.png)

仕上がりを確認したら、左の `保存` ボタンで Question として永続化されます。後から `Question` タブの一覧で `編集` を押せばクエリ・グラフ設定の両方を更新できます。

### 9.3 Dashboard にカードとして並べる

`Dashboard 作成` から名前を付け、`+ カード追加` で **既存の Question を 1 カードずつ** 配置していきます。`カスタムタイトル` を入れるとカード見出しを上書きできます。保存後にダッシュボード一覧から開くと、各カードが Question を実行してグラフを描画します。
