...
本マニュアル内のスクリーンショットは 2026年3月8日に Playwright で再取得したものです。  
...
- `初期値`: テキスト型では「なし／ユーザー名／ユーザー所属／自由入力」を横並びで選択できます
...
- `回答者の電話番号を自動入力する`: チェックすると回答者のアカウント情報から電話番号を自動入力します
...
| メールアドレス | メール形式のみ受け付ける入力。回答者のアドレスを自動入力するオプションあり |
| URL | URL 形式のみ受け付ける入力 |
...
- `初期値（ユーザー名 / ユーザー所属 / 自由入力）`: テキストで利用できます
- `初期値を現在の日付/時刻にする`: 日付・時間で利用できます
- `回答者のメールアドレスを自動入力する`: メールアドレスで利用できます
- `回答者の電話番号を自動入力する`: 電話番号で利用できます
