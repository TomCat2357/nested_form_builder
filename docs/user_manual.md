...
| 標準様式出力ファイル名規則 | 出力ファイル名のテンプレートです。`{ID}`, `{YYYY}`, `{MM}`, `{DD}`, `{H}`, `{m}`, `{s}`, `{gg}`, `{フィールド名}` を使えます。既定値は `{ID}_{YYYY}-{MM}-{DD}` です |
...
| フォルダ命名規則 | レコードごとの子フォルダ名テンプレートです。`{ID}`, `{YYYY}`, `{MM}`, `{DD}`, `{H}`, `{m}`, `{s}`, `{gg}`, `{フィールド名}` を使えます。空白の場合は子フォルダを作らずルートフォルダ直下に保存します |
...
| `{gg}` | 元号（平成、令和 など） |
| `{ee}` / `{e}` | 和暦年（0 埋め / そのまま） |
| `{_NOW}` | 現在日時（`YYYY-MM-DD HH:mm:ss` 形式）。`date:` や `time:` 変換器と組み合わせて使えます |

`{_NOW}` の使用例:

```text
{_NOW|date:YYYY年M月D日}     → 2026年4月4日
{_NOW|date:gge年M月D日}      → 令和8年4月4日
{_NOW|time:HH:mm}            → 10:20
{_NOW|date:YYYY/MM/DD(ddd)}  → 2026/04/04(土)
```
...
| 変換器 | 構文例 | 出力例 | 説明 |
| --- | --- | --- | --- |
| `date` | `{生年月日\|date:YYYY/MM/DD}` | `2000/01/15` | 日付フォーマット |
| | `{生年月日\|date:gge年M月D日}` | `平成12年1月15日` | 和暦フォーマット |
| | `{入社日\|date:YYYY/MM/DD(ddd)}` | `2026/04/04(土)` | 曜日付き（ddd=短縮、dddd=完全） |

使えるトークン: `YYYY`, `YY`, `MM`, `M`, `DD`, `D`, `gg`（元号）, `gge`（元号+年）, `ee`, `e`, `ddd`（月）, `dddd`（月曜日）

##### 時刻

| 変換器 | 構文例 | 出力例 | 説明 |
| --- | --- | --- | --- |
| `time` | `{受付時間\|time:HH時mm分}` | `14時30分` | 時刻フォーマット |
| | `{受付時間\|time:HH:mm:ss}` | `14:30:00` | 秒付きフォーマット |

使えるトークン: `HH`（0 埋め時）, `H`（時）, `mm`（0 埋め分）, `m`（分）, `ss`（0 埋め秒）, `s`（秒）

対応する入力形式: `HH:mm`, `HH:mm:ss`, `YYYY-MM-DD HH:mm:ss` など
