...
| 項目 | 説明 |
| --- | --- |
| 印刷様式テンプレートURL | Google ドキュメントの URL を指定します。未設定時は自動生成ドキュメントを使います |
| 印刷様式出力ファイル名規則 | 出力ファイル名のテンプレートです。`{@_id}`, `{@_NOW}`, `{@_NOW\|time:YYYY}`, `{@フィールド名}` 等のトークンを使えます。既定値は `{@_id}_{@_NOW\|time:YYYY-MM-DD}` です |
...
| フォルダ命名規則 | レコードごとの子フォルダ名テンプレートです。`{@_id}`, `{@_NOW}`, `{@_NOW\|time:YYYY}`, `{@フィールド名}` 等のトークンを使えます。空白の場合は子フォルダを作らずルートフォルダ直下に保存します |
...
- `出力ファイル名`: ファイル名テンプレートを設定できます。未指定時はフォーム設定の印刷様式出力ファイル名規則が使われます
...
予約トークンは `if` 条件で真偽判定にも使えます: `{@値|if:@_folder_url,代替値}`
...
| `noext` | `{添付ファイル\|noext}` | `報告書` | ファイル名から拡張子を除去（カンマ区切り複数対応） |
...
| 変換器 | 構文例 | 出力例 | 説明 |
| --- | --- | --- | --- |
| `if` | `{@性別\|if:@性別==男,Ms.}` | `男` / `Ms.` | 条件が真ならパイプ値を返し、偽なら elseValue を返す |
| | `{@備考\|if:@備考,なし}` | `メモあり` / `なし` | 値の有無で判定（truthiness チェック） |
| `default` | `{電話\|default:未入力}` | `未入力` | 値が空のときにフォールバック |
| `map` | `{評価\|map:A=優;B=良;C=可;*=不明}` | `優` | 値を別の文字列に変換。`*` はデフォルト |

##### if 変換の条件式

`{field|if:condition,elseValue}` — 条件が真ならパイプ入力値をそのまま返し、偽なら elseValue を返します。
...
- `@フィールド名` — 他のフィールドの値を参照
- `@_予約トークン` — 予約トークン（`@_folder_url` 等）を参照
- `"リテラル"` — ダブルクォート囲みの文字列
- 数字 — 数値として比較
