...
- **読取経路での復元**: `driveFileUrl` は fileId から都度組み立てる（`https://drive.google.com/file/d/<fileId>/view`）。forms は `Forms_normalizeMappingValue_`、questions/dashboards は共通コア `Nfb_normalizeMappingValue_`（`Analytics_getMapping_` が `Nfb_normalizeMapping_(mapping, "name")` 経由で読取時に適用）が再構築する。
