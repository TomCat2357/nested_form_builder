...
- **フォルダ込み名（`フォルダ/サブ/名前`、正規の区切りは `/`）が正規の識別名**。入力では `/`（正規）と `|` の両方を階層区切りとして受理し、正規化（`normalizeFolderPath` / `Forms_normalizeFolderPath_`）で常に `/` 区切りへ揃える（`hasFolderDelimiter` がフォルダ込み名の判定に使われる）。保持名・SQL のフォーム参照（`FROM [フォルダ/フォーム名]`、`FROM [フォルダ|フォーム名]` も可）はフォルダ込みで持ち、別フォルダの同名を一意に識別する。バレ名（フォルダなし）はその名前が全体で一意のときのみ解決し、同名複数のバレ名は曖昧として解決しない（フォルダ込み指定を促す）。フロントは `formQualifiedName` / `buildFormIndex.byPath`、GAS は物理相対フォルダ＋葉名のキーで解決する。残る真の重複は「同一フォルダ＋同名」だけになり、これは同期が最新を残して整理する。
