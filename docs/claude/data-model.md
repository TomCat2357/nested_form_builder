...
- `pid` — 親レコード ID（任意）。常設の固定メタ列。URL に `?pid=<値>` を付けると、その pid に等しい行だけを一覧し、その状態で追加した新規行には pid を必ず刻む（`Code.gs` が `window.__PID__` に注入 → `gasClient` が各レコード API へ付与 → `Nfb_resolvePidFromCtx_` / `Nfb_recordMatchesPid_` でサーバ側フィルタ・刻印）。

固定列の定義は `gas/constants.gs` の `NFB_FIXED_HEADER_PATHS`。先頭 8 列（`id`〜`deletedBy`）の順序は `sheetsRowOps.gs` / `codeSyncRecords.gs` の位置決め書き込み（`rowData[0..7]`）が依存するため不変。`pid` は末尾に置き、シートに無ければ `Sheets_ensureHeaderMatrix_` が末尾へ列を挿入する（既存列は移動しない）。
