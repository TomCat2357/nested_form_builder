...
選択肢系フィールド（`radio` / `select` / `checkboxes` / `weekday`）は **フィールドごとに 1 列**（葉＝フィールドのラベル）で保存する（view 形式に一本化。旧・選択肢ごとの `親|選択肢` 真偽値列は廃止）。セルには選択ラベルが入り、複数選択（`checkboxes`）は共有 codec `builder/src/utils/multiValue.js`（`joinMultiValue` / `splitMultiValue`）で **カンマ `,` 連結・ラベル内の `,`/`\` はバックスラッシュエスケープ**して 1 セルに格納する。検索（MV_EQ/MV_IN）・分析 view 行・再読込もこの codec を共有する。`gas/sheetsHeaders.gs` の `Sheets_buildOrderFromSchema_` が選択肢を単一列で出力する。
