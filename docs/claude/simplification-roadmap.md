...
**C-2. choju ドメイン定数の二重定義（取り下げ・未対応）** — `choju_kyokasho`(出力) と `choju_yoshiki`(取込) で**接頭辞以外バイト一致**だった法令由来語彙
`SPECIES_ORDER_` / `TOOL_ORDER_` / `TOOL_KIND_` / `GUN_LIC_` を一度 `shared/chojuDomain.json` 単一ソース＋ sync スクリプトに集約したが、両モジュールを独立して扱う方針のため**リバートして各 `Combined.gs` 内のインライン定義に戻した**。法令由来で値は一致しているが、各モジュールが自分用の定義を持つ。
...
- これらを本気で共通化するなら、「`shared/` を単一ソースにビルド注入」方式（C-2 で試行・取り下げ）を汎用ヘルパーへ拡張するのが筋。ただし独立デプロイの自己完結性とのトレードオフに注意。
