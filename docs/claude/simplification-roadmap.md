...
## 2026-06-28 冗長監査（直近2日のコミット `dc81c60..HEAD`、49 コミット・`+11,580/-4,665`）

3 領域（`gas/` 本体 / `builder/src/` フロント / `gas_for_external_action/` 外部アクション）を並行監査した結果。

### A. `gas/`（本体バックエンド）— クリーン
期間内のリファクタコミットで解消済み（`9747884` 44 ワンライナー削除 / `5d14810` ロック+safeCall を `Nfb_withLockedSafeCall_` に集約 / `ba1f50c` admin プロパティ get/set 共通化 / `1c50eb0` `Forms_makeStateRun_` / `ad9beb1` `Nfb_trimStr_` 採用 / `ec3c59a` 汎用エラーメッセージ定数化）。新規冗長なし。残る `FormsApi_SetArchived_` / `FormsApi_SetReadOnly_`（`formsPublicApi.gs`）はフラグ正規化の責務があり**削除非推奨**。

### B. `builder/src/`（フロント）— クリーン
`e19c01e`「冗長コードを共通化」と `b0732d2` 巨大ページ構造分離で分割し残しなし。`QuestionEditorPage`(-620) / `useSearchPageState`(-355) / `PreviewPage`(-315) / `DashboardEditorPage` の hook・helper 抽出は import 整合済み。新規 util（`utils/strings.js` `utils/spreadsheet.js` `utils/openWindow.js` `app/components/DialogFooter.jsx` `core/fieldTypeSets.js`）は複数箇所で実使用。デッドコード・複製なし。

### C. `gas_for_external_action/`（外部アクション）— 冗長集中・本監査で一部対応

各モジュールは clasp で**独立した GAS プロジェクト**として push される自己完結 `Combined.gs`（共有コード注入のビルド機構なし）のため、モジュール間重複は構造的側面がある。

**C-1. デッドコード削除（対応済み）** — 今回範囲で追加された未使用の内部ヘルパー（末尾 `_`・呼び出し 0）を削除:
- `choju_kyokasho/Combined.gs`: `Cho2_a1Offset_` / `Cho2_hasChoice_` / `Cho2_escapeHtml_`
- `choju_yoshiki/Combined.gs`: `Cho_comparePinkDate_` / `Sheets_isValidDate_`
- 参考（範囲外＝既存デッドコード、未対応）: `choju_yoshiki/Combined.gs` の `Cho_splitChecks_`。

**C-2. choju ドメイン定数の二重定義（対応済み・単一ソース化）** — `choju_kyokasho`(出力) と `choju_yoshiki`(取込) で**接頭辞以外バイト一致**だった法令由来語彙
`SPECIES_ORDER_` / `TOOL_ORDER_` / `TOOL_KIND_` / `GUN_LIC_` を、`gas_for_external_action/shared/chojuDomain.json` を単一ソースに集約。各 `Combined.gs` の
`// >>> shared:chojuDomain` 〜 `// <<< shared:chojuDomain` マーカー区間を
`gas_for_external_action/scripts/sync-shared-constants.mjs` が接頭辞付きで再生成する（`--check` で同期検査）。各モジュールは自己完結のまま（実行時の共有ライブラリ依存なし）。`ROSTER`（名簿の幾何）は読む列がモジュールごとに異なるため**共有しない**。

**C-3. モジュール間ユーティリティ重複（記録のみ・将来候補）** — 独立デプロイ＋`template/` 由来のため今回は対象外:
- `*_hmacHex_`（HMAC-SHA256）: choju_kyokasho / choju_yoshiki / kujo_intake / `template/Code.gs` の 4 重
- `parsePayload_` 4 重 / `escapeHtml_` 4 重 / 文字列正規化 `*_str_`・`*_normalizeText_` 3 重 / Script Properties アクセサ `*_getProp_` 3 重
- ctx トークン戦略の不統一: choju 系は CacheService（TTL 失効リスク）、kujo_intake は自己完結 base64url。将来 kujo 方式へ寄せる余地。
- これらを本気で共通化するなら、C-2 と同じ「`shared/` を単一ソースにビルド注入」方式を汎用ヘルパーへ拡張するのが筋。
