...
- `choju_intake/Combined.gs`: `Cho_comparePinkDate_` / `Sheets_isValidDate_`
- 参考（範囲外＝既存デッドコード、未対応）: `choju_intake/Combined.gs` の `Cho_splitChecks_`。

**C-2. choju ドメイン定数の二重定義（取り下げ・未対応）** — `choju_kyokasho`(出力) と `choju_intake`(取込) で**接頭辞以外バイト一致**だった法令由来語彙
...
- `*_hmacHex_`（HMAC-SHA256）: choju_kyokasho / choju_intake / kujo_intake / `template/Code.gs` の 4 重
