...
>   - 葉ラベル単独参照は廃止。同名の葉ラベルが複数階層に存在し得るため、参照は常に一意のフルパスで明示する（検索バーと同じセマンティクス。`docs/claude/search-query-syntax.md` の「列名の書き方」と同一規則・同一コーデック `pathCodec.js`）。
...
>   - 列名・自フォーム内の親子 JOIN（`pid`）の書き方は検索/Question SQL と同一（`[列名]` / バッククォート。`docs/claude/search-query-syntax.md` / `docs/notebooklm/question-sql-rulebook.md`）。`_id` は文字列リテラル・コメント・`[...]`・`` `...` `` の内側では置換されない（`` `_id` `` は従来どおり予約トークン）。
>   - **結果の畳み込み**: 0 行 → 空文字 / 1 行 1 列 → そのスカラ / それ以外 → 全セルを行優先で `coerceResultToString` し、空も含めて位置を保ったまま `, ` 連結（NULL/空セルの行を詰めない）。カンマ列リスト分割は **適用されない**（SELECT リストのカンマを壊さない）。
>   - **ネスト `{{SELECT {{...}} FROM yyy}}`**: SQL 本文の内側にさらに `{{...}}`（単純式 / 別の full-query どちらも可）を書くと、内側から先に評価され、その結果が SQL 文字列リテラル（`'...'`、`'` は `''` にエスケープ）として親の SQL 本文へ埋め込まれてから実行される（例: 内側の結果が `hogehoge` なら `{{SELECT {{...}} FROM yyy}}` は実行時に `SELECT 'hogehoge' FROM yyy` として評価される）。深いネストも再帰的に対応。実装は `features/expression/fullQuerySql.js` の `resolveNestedBraceTokens` / `sqlStringLiteral` と、それを呼び出す `utils/tokenReplacer.js` の `prefetchQueryTokens`（`evalNestedToken_`）。クライアント側のみで完結する（GAS 側の変更は不要）。
...
>       - **統一契約（完全統一）**: fileUpload 項目の行値は **全経路で「保存 JSON 文字列」に統一**（本番プレビュー/印刷・GAS 出力・Playground・full-query・Question SQL・検索）。よって **素の参照** `` {{`添付`}} `` は保存 JSON 文字列そのもの（`{"files":[{"name","driveFileId","driveFileUrl"}],"folderUrl"?,"folderName"?}` / フォルダ無しは裸配列 JSON）を返す。各パーツはこれらの UDF で取り出す: `FILE_NAMES` = カンマ区切りファイル名、`FILE_URLS` = カンマ区切り driveFileUrl、`FOLDER_NAME` / `FOLDER_URL` = フォルダ名 / URL。UDF は `registerNfbUdfs.js` の `normalizeFileUploadCell` が保存文字列（裸配列 JSON / オブジェクト JSON）・レガシー in-memory 配列・オブジェクトを吸収して parse する。
>       - **`FILE_NAMES` は拡張子付きの生名を返す**（`hideFileExtension` に依存しない＝コンテキスト非依存）。拡張子を落とすには `` {{NOEXT(FILE_NAMES(`添付`))}} `` のように `NOEXT` で包む。
>       - 統一の実装: フロント行ビルダは `labelValueMap.js` `buildFileUploadRowEntries`（`collectFileUploadMeta` が `serializeFileUploadValue` で用意する `meta.storageValue` を path に載せる）、view 行は `entriesToViewRows.js` が保存文字列を素通し、GAS ツインは `templateEvaluator.gs` `nfbTplBuildFileUploadRowEntries_`（クライアント `storageValue` 優先。コピー/移動後の URL 復旧時は `driveOutput.gs` `Nfb_resolveFileUploadMetaUrls_` が復旧後の meta 配列から `storageValue` を再構築）。素参照が JSON になるのは `coerceResultToString` が文字列を素通しするため（両ランタイム）。
