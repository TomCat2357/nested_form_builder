...
> - **フィールド参照** はバッククォートで囲む（` `` `）。
>   - ネストされた子質問は `親|子|孫` のフルパスでバッククォートで囲んで参照する（例: `` `設置場所|設置開始日` ``）。トップレベル質問はパス＝葉ラベル。階層区切りは `|`（正規）と `/` の両方を受理する（`` `設置場所/設置開始日` `` も同じ列に解決）。`headerKeyToAlaSqlKey` が `|` と `/` をどちらも `__` に揃えるため、列インデックス側（正規パス）とユーザー入力側（`親|子` / `親/子`）が同一キーへ収束する（SQL・検索・テンプレート・計算項目に共通）。
