...
>
> 旧 `{@field|pipe}` 構文（パイプ変換）と `[...]` JavaScript 演算式は廃止され、`{ alasql 関数式 }` に統一済み。
...
> - **単一ブレース `{...}`（data モード） vs 連続二重ブレース `{{...}}`（view モード）**: 内側式・UDF・カンマ列リスト規則は完全に共通で、**違いはフィールド参照が解決される行だけ**。
>   - `{...}`（data モード）= **元データ形式**の行で評価。選択肢はオプション単位パス `` `親|選択肢` `` → 真偽値、非選択肢は canonical 保存値（`context.dataValueMap` 由来）。
>   - `{{...}}`（view モード）= **ビュー形式**の行で評価。選択肢はラベル連結、表示用文字列に揃った値（`context.labelValueMap` 由来）。
>   - 例: 単一選択肢 `性別` で「男性」を選んだレコードなら `` {`性別|男性`} `` → `"true"`、`` {{`性別`}} `` → `"男性"`。
>   - 置換フィールド・印刷様式・Gmail 等すべての置換経路（フロント / GAS 双方）に適用。スキャナ（`templateScanner.js` の `describeToken` / GAS 側双子 `gas/templateEvaluator.gs`）が `{{ }}` を `mode="view"` と判定し、`resolveTemplate` が `viewRow` 行を使う。`viewRow` 未指定（旧コンテキスト）は data 行にフォールバックして従来挙動を保つ。
>   - 実装: フロント `utils/tokenReplacer.js` の `buildTemplateRow` が data / view 2 本の行を作り `resolveTemplate(text, dataRow, { viewRow })` に渡す。挙動は `templateScanner.test.js` / `templateEvaluator.test.js` / `tests/gas-drive-template-replacement.test.cjs` が担保。
