...
各タイプのスキーマ上の属性・ネスト構造・選択肢の扱いは [`data-model.md`](./data-model.md) も参照。`substitution` は `templateText` で `{...}`（元データ形式）/ `{{...}}`（ビュー形式）の alasql 式を使い、他フィールド値を組み合わせた文字列・数値計算結果を表示するフィールド（旧 `[...]` ブラケット演算式は廃止。トークン構文は [`drive-template-tokens.md`](./drive-template-tokens.md) 参照）。
