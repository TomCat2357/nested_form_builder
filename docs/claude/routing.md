...
`text` / `number` / `email` / `phone` / `url` / `date` / `time` / `radio` / `select` / `checkboxes` / `textarea` / `regex` / `userName` / `fileUpload` / `message` / `printTemplate` / `webhook` / `formLink` / `substitution`（旧 `weekday`（曜日）型は廃止）

`formLink`（「別フォームを開く」）は値を持たないアクションボタン。`childFormId`（リンク先フォームの fileId）＋ `childFormPath`（論理パス・表示用）を持ち、押下で別タブに `?form=<childFormId>&pid=<このレコードのID>` を開く（＝そのレコードに紐づく子フォーム）。`webhook` と同じくデータ列を生成しない。
