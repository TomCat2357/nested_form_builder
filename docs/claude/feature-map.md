...
│  Pages（画面）                                                   │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌────────────────┐     │
│  │HomePage  │ │FormPage  │ │SearchPage│ │SettingsPage    │     │
│  │フォーム/ │ │入力・編集│ │検索・一覧│ │統合設定         │     │
│  │ダッシュ  │ │          │ │          │ │一般/管理者タブ  │     │
│  │タブ      │ │          │ │          │ │                 │     │
│  └──────────┘ └──────────┘ └──────────┘ └────────────────┘     │
│  ┌──────────────┐ ┌────────────────┐                           │
│  │AdminHubPage  │ │FormSettingsPage│                           │
│  │管理ハブ      │ │フォーム別設定  │                           │
│  │(3カード)     │ │                │                           │
│  └──────────────┘ └────────────────┘                           │
│  ┌─ pages/admin/ (管理者専用) ────────────────────────────┐    │
│  │ AdminFormListPage      フォーム管理一覧                │    │
│  │ AdminFormEditorPage    フォーム編集                    │    │
│  │ AdminDashboardListPage ダッシュ管理一覧                │    │
│  │ AdminQuestionListPage  Question 管理一覧               │    │
│  │ DashboardEditorPage    ダッシュ編集                    │    │
│  │ QuestionEditorPage     Question 編集                   │    │
│  └─────────────────────────────────────────────────────────┘    │
│  ┌─ pages/dashboards/ ────────────────────────────────────┐    │
│  │ DashboardViewPage      ダッシュボード閲覧（誰でも可）   │    │
│  └─────────────────────────────────────────────────────────┘    │
├─────────────────────────────────────────────────────────────────┤
│  Features（機能モジュール）                                      │
│                                                                 │
...
│          formsCache / analyticsCache（IndexedDB一覧キャッシュ）   │
│          recordsMemoryStore（回答はメモリ常駐・IndexedDB非使用） │
│          uploadQueue / uploadWorker（オフライン保存 write-behind）│
...
│  ┌─ テンプレートシステム ──────────────────────────┐            │
│  │ トークン置換: {{`field`}}, {{func(`field`)}} (alasql) │         │
│  │ {{...}}=ビュー形式に統一（{...} は廃止＝リテラル）  │           │
│  │ 予約トークン: {{`_id`}}, {{`_record_url`}} 等       │           │
│  │ 現在時刻: NOW() → "YYYY-MM-DD_HH:mm:ss.sss"       │           │
│  │ 和暦対応: {{TIME_FORMAT(NOW(), 'gg ee年')}}        │           │
│  └──────────────────────────────────────────────────┘           │
│  ┌─ ドキュメント出力 ──────────────────────────────┐            │
│  │ Google Docテンプレートからの文書生成               │           │
│  │ PDF / Google ドキュメント / Gmail下書き            │           │
│  │ バッチ出力（複数レコード一括）                     │           │
│  └──────────────────────────────────────────────────┘           │
│  ┌─ フォルダ・ファイル管理 ────────────────────────┐            │
│  │ テンプレート名によるフォルダ自動生成               │           │
...
│  bundle.js      FILE_ORDER 53 ファイル（*.gs + 純関数 *.js +    │
│                 生成 UDF）→ dist/Bundle.gs 結合                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│  recordsMemory…  ├─ sync_records / save_lock ───►│  Sheets_*        │
...
│                  │                               │                  │
│  SettingsAdminTab├─ admin_key / admin_email ────►│  settings.gs     │
│  SettingsPage    │  nfbGetRestrictToFormOnly      │  properties.gs   │
├──────────────────┤                               ├──────────────────┤
