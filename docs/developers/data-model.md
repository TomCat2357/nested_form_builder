...
- フォルダ走査・同期はファイル名で行い、`mapping[fileId] = { fileId, name, folder }`（id ＝ fileId・永続化時は `driveFileUrl` を捨てる）として **Script Properties registry** にレジストリ（耐久バックストップ）を保持する。フロントの参照解決・一覧表示は **IndexedDB registry**（`builder/src/app/state/registryStore.js`・作業キャッシュ）が別途ミラーする。両者の関係は「論理＝正本／物理＝キャッシュ」「Props 最小＋IndexedDB キャッシュの二層」（[links-and-save.md](links-and-save.md) §6）。
- リンク（クエスチョン→フォーム `formId` / ダッシュボード→クエスチョン `questionId`）は id（fileId）で解決し、解決できないときは registry の `folder＋名前` アンカーと home の `*Path`（`formPath`/`questionPath` 等）で標準フォルダ構成内を解決する（`Forms_resolveFormRef_` / `Analytics_resolveQuestionRef_`）。**相手の名前（`formName`/`questionName`/`childFormName`）の二重持ちと読みフォールバックは撤去済み**（links-and-save.md §6-2）。
...
- 構成コピー（別ルートへ複製）では全ファイルが新 fileId になるため、**コピー時に物理 ID（エンティティ参照 / spreadsheet / 印刷様式）を全消去して論理パス（`*Path`）だけを残し、コピー先で再解決する**。コピー先の取り込み（`StdFolders_importMapping_`）が `Admin_rebuildRegistryFromLogical_`（registry 充填）→ `Admin_reresolveAllRefsFromLogical_`（`*Path`→fileId 貼り直し）を初回解決ゲートとして通す（links-and-save.md §6-5）。
...
かつて設定＞管理にあった「同期（フォルダ走査）」と「構成レポート（リンク診断）」、および単体の「参照の再リンク」「同名フォーム重複整理」ボタン（公開アクション `std_folders_rebuild_map` / `std_folders_link_report` / `std_folders_relink_refs` / `std_folders_dedupe_forms`）はいずれも廃止された（オフラインファースト保存への移行に伴う）。

壊れた参照の再リンクと同名重複の整理は、フォーム/Question/Dashboard の保存時にサーバ側で走る自動リンク補完（`StdFolders_alignReferencesOnSave_`）が担う。読取時に fileId の物理ファイルが消失していたときの解決は、上記の `Analytics_resolveItemFileOrNull_`（論理パス＝名前での引き当て直し）のとおり。
...
- `pid` — 親レコード ID（任意）。メタ列ブロックの最後（`deletedBy` の直後・データ列の前）に置く常設の固定メタ列。URL に `?pid=<値>` を付けると、その pid に等しい行だけを一覧し、その状態で追加した新規行には pid を必ず刻む（`Code.gs` が `window.__PID__` に注入 → `gasClient` が各レコード API へ付与 → `Nfb_resolvePidFromCtx_` / `Nfb_recordMatchesPid_` でサーバ側フィルタ・刻印）。
  - **クエリでの親子連結**: 「別フォームを開く」(`formLink`) で開いた子フォームのレコードは `pid == 親レコードの id`。`pid` は AlaSQL の view 行・列インデックス（`builder/src/features/analytics/entriesToViewRows.js` / `utils/columnIdentifierResolver.js`）にもメタ列として露出するので、Question SQL でも検索の SQL モード（先頭 `SELECT`）でも `JOIN ... ON c.pid = p.id` や `WHERE [id] IN (SELECT pid FROM [子] ...)` で親子をまたいだ集計・絞り込みができる。formLink 項目名から子フォーム名・件数を取る `CHILD_FORM_NAME` / `CHILD_FORM_COUNT` UDF も使える（`docs/notebooklm/question-sql-rulebook.md` 3.2.1 / 3.2.2、`docs/developers/search-query-syntax.md`）。

固定列の定義は `gas/constants.gs` の `NFB_FIXED_HEADER_PATHS`。**この配列順がそのまま物理列の先頭固定位置**（メタ列 i → 物理列 i+1）。`Sheets_ensureHeaderMatrix_` → `Sheets_repairMetaColumnPositions_` が全メタ列を一律に正規位置へ揃える（欠落は正規位置へ挿入、位置ずれは移動。`pid` だけの特別処理は持たない）。既存シートで `pid` が無ければ `deletedBy` の直後へ挿入され、既存データ列は右へシフトする。先頭 8 列（`id`〜`deletedBy`）の順序は `sheetsRowOps.gs` / `codeSyncRecords.gs` の位置決め書き込み（`rowData[0..7]`）が依存するため不変。

ファイルアップロード列のセル JSON には `folderUrl` が埋め込まれており、`parseFileUploadStorage()` で取り出せる。検索表の「フォルダを開く」アクションや印刷時のフォルダコンテキストはここから派生させる。

### 動的列

質問パスを **スラッシュ `/`** で連結して列名（内部キー）とする。例: `parent/child/question`。
ヘッダーの 11 行は、ルートから末端質問までの各階層を 1 セルずつ占める（＝列キーの実体はセグメント配列で、`/` 連結は文字列化のときだけ。フィールド名自体に `/` が含まれる場合はバックスラッシュでエスケープして往復する。共有コーデック `builder/src/utils/pathCodec.js` / GAS 双子 `gas/pathCodec.gs`）。`|` は区切りではなく通常文字（旧データの `親|子` 列参照は `headerKeyToAlaSqlKey` が後方互換で `__` に解決）。
...
- **列ごとのセル書式（型）**: 書き込み時にフィールド型ごとの数値書式を割り当て、Sheets が `"1-1"` を日付（`2026-01-01`）に、`"007"` を数値に勝手に変換するのを防ぐ。自由記述系（`text` / `textarea` / `regex` / `url` / `userName` / `email` / `phone` / `fileUpload` / `substitution`）と選択肢マーカー列は **`"@"`（プレーンテキスト）**、`number` は数値のまま（集計のため）、`date` / `time` は数値の日時シリアル値＋専用書式。書式マップは `gas/sheetsHeaders.gs` の `Sheets_collectColumnFormatMap_`（`Sheets_buildOrderFromSchema_` と同じ列キー規則）で作り、`Sheets_upsertRecordById_` / 同期書き込み（`gas/codeSyncRecords.gs`）が **`setValues` より前に** `setNumberFormats` で適用する（テキスト列を先に `"@"` にしないと文字列が変換されてしまうため）。書式はセル単位で永続するので、以後のパージ詰め直しや再編集でも型が保たれる。
