...
function renderPreview(res){EXCEL=res.excelFileId||"";var f=res.friendly||{};var ap=f.applicant||{};var html="";html+="<div class='sec'><label class='chk'><input type='checkbox' id='chkParent' checked> <b>この申請を取り込む</b>（"+esc(ap.type||"")+(ap.name?(" / "+esc(ap.name)):"")+"）</label>"+tbl(ap.rows||[])+"</div>";var ws=f.workers||[];html+="<h2>従事者（"+ws.length+" 名）</h2>";for(var i=0;i<ws.length;i++){var w=ws[i];html+="<div class='sec'><label class='chk'><input type='checkbox' class='chkChild' data-idx='"+w.index+"' checked> <b>"+esc(w.title)+"</b></label>"+tbl(w.rows||[])+"</div>";}$("preview").innerHTML=html;$("commit").style.display="";$("commitWrap").style.display="block";}
