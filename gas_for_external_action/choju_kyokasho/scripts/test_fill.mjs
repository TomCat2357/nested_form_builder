...
eq("workerNo No.採用", C.Cho2_workerNo_(["5", "8"], 1), "8");        // No. 列の値をそのまま
eq("workerNo 空→位置代替", C.Cho2_workerNo_(["", "8"], 0), "1");      // 空なら 1 始まり位置
eq("workerNo 配列なし→位置代替", C.Cho2_workerNo_(undefined, 2), "3");
eq("sheetLabel 番号あり", C.Cho2_sheetLabel_("5", "田中", "従事者3"), "5_田中");
eq("sheetLabel 番号空→名前のみ", C.Cho2_sheetLabel_("", "田中", "従事者3"), "田中");
eq("sheetLabel 名前空→fallback", C.Cho2_sheetLabel_("5", "", "従事者3"), "5_従事者3");
...
eq("個人 許可証0 label", planP.kyokasho[0].label, "1_鈴木新之助"); // 許可証_<No.>_<氏名>（No.=marker #1）
eq("個人 許可証1 label", planP.kyokasho[1].label, "2_田中聡"); // No.=marker #2
...
eq("法人 従事者証0 label", planC.juji[0].label, "1_鈴木新之助"); // 従事者証_<No.>_<氏名>（No.=marker #1）
eq("法人 従事者証1 label", planC.juji[1].label, "2_田中聡"); // No.=marker #2
eq("法人 許可証0 label(法人名のまま)", planC.kyokasho[0].label, "株式会社ハンター協会"); // 法人許可証は 1 通・No. 無し
...
// ---- 従事者番号 = No. 列（marker が並び順と不一致でも許可証番号・シート名に No. を反映）----
const gapItems = toItems(
  parentItems([
    ["申請者情報/個人・法人の別", "個人"],
    ["申請者情報/個人・法人の別/個人/氏名", "鈴木新之助"],
    ["申請者情報/個人・法人の別/個人/住所", "小樽市"],
    ["申請者情報/個人・法人の別/個人/生年月日", "1996-05-01"],
  ]).concat(w1Items("従事者情報/#5/")).concat(w2Items("従事者情報/#8/")) // No.=5,8（位置は 0,1）
);
const pGap = C.Cho2_parseRecordItems_(gapItems);
eq("No.不連続 workerNos", JSON.stringify(pGap.workerNos), JSON.stringify(["5", "8"]));
const planG = C.Cho2_buildPlan_(pGap, "");
// 個人許可証: シート名・C3 とも No. 駆動（"1-1-5" / "1-1-8"）
eq("No.不連続 許可証0 label", planG.kyokasho[0].label, "5_鈴木新之助");
eq("No.不連続 許可証0 C3", cellOf(planG.kyokasho[0].cells, "C3"), "1-1-5");
eq("No.不連続 許可証1 label", planG.kyokasho[1].label, "8_田中聡");
eq("No.不連続 許可証1 C3", cellOf(planG.kyokasho[1].cells, "C3"), "1-1-8");
// 従事者名簿 E 列も No. 駆動
eq("No.不連続 名簿 E5(1人目)", cellOf(planG.roster[0].cells, "E5"), "1-1-5");
eq("No.不連続 名簿 E14(2人目)", cellOf(planG.roster[0].cells, "E14"), "1-1-8");
// 通知の許可証番号レンジは先頭〜末尾従事者の No.
eq("No.不連続 通知 範囲C14", cellOf(planG.shinko, "C14"), "1-1-5");
eq("No.不連続 通知 範囲E14", cellOf(planG.shinko, "E14"), "1-1-8");
// 法人従事者証も No. 駆動（C4・シート名）
const planGc = C.Cho2_buildPlan_(C.Cho2_parseRecordItems_(toItems(
  parentItems([
    ["申請者情報/個人・法人の別", "法人"],
    ["申請者情報/個人・法人の別/法人/法人名", "株式会社ハンター協会"],
    ["申請者情報/個人・法人の別/法人/代表者名", "代表取締役　秋　はじめ"],
    ["申請者情報/個人・法人の別/法人/住所", "札幌市"],
  ]).concat(w1Items("従事者情報/#5/")).concat(w2Items("従事者情報/#8/"))
)), "");
eq("No.不連続 従事者証0 label", planGc.juji[0].label, "5_鈴木新之助");
eq("No.不連続 従事者証0 C4", cellOf(planGc.juji[0].cells, "C4"), "1-1-5");
eq("No.不連続 従事者証1 label", planGc.juji[1].label, "8_田中聡");
eq("No.不連続 従事者証1 C4", cellOf(planGc.juji[1].cells, "C4"), "1-1-8");
