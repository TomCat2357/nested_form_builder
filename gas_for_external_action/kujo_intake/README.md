...
1. **検索一覧の外部アクションボタン**（例「末端問い合わせ csv 取り込み」）。本体 GAS がサーバ間リレーで
   この `doPost` を叩くと、`doPost` は**取り込み（パース）ページの URL を `openUrl` で返す**。本体フロントが
   それを新しいタブで開く（choju と同方式）。＝ボタン押下で取り込み画面が開く。検索結果 payload は使わない。
   - 外部アクションの **URL は本アプリの `/exec`**（パラメータ不要）。`/dev` は所有者専用なので**必ず `/exec`** を貼ること。
2. **URL を直接開く**: `<exec>?page=parse`（PDF は `&pdf=<DriveのURL/ID>` で prefill・手貼りも可）。

> doPost は **Sheets に書かない**（JSON を返すだけ）。本送信の前に本体が **誤送信防止プローブ**（`nfbProbe=1`）を
> 送ることがあり、その場合は共有シークレットの HMAC 署名を返す（§3-2）。シークレット未設定なら直接送信（既定）。
...
2. **設定登録（任意）**: GAS エディタで
   ...
   - 第 3 引数 `extActionSecret` は**誤送信防止プローブの共有シークレット**（任意）。本体 GAS の
     `NFB_EXT_ACTION_SECRET` と**同じ値**を入れると、本体は本送信の前に nonce プローブで宛先を検証する。
     空（既定）なら本体はプローブを送らず直接送信（後方互換）。引数省略時は既存値を据え置く。
3. **ウェブアプリとしてデプロイ**（アクセス: 全員 / 実行: 自分）。`/exec` URL を控える。
4. **検索一覧の外部アクションを設定**（CSV/PDF 取り込みボタン用）: 親フォームの「外部アクション（検索）」に
   ラベル（例「末端問い合わせ csv 取り込み」）と **URL = 本アプリの `/exec`**（パラメータ不要）を登録し、`管理者のみ` に。
   ボタン押下 → `doPost` が `openUrl=<exec>?page=parse` を返す → 取り込み画面が新しいタブで開く。
5. （直接開く場合）`<exec>?page=parse` → **PDF** はリンク（`?pdf=<DriveのURL/ID>` で prefill・手貼りも可）、
   **CSV** は「CSV ファイル」選択（複数可）→ [パース] → プレビューと JSON を確認。
6. ネスト分類（相談大分類など）を人手で補完 → JSON を **管理者 > Playground** に貼って `sync_records` 実行。
