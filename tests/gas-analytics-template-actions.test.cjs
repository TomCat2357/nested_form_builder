...
test("Analytics_saveTemplate_ は2回目以降の保存で既存ファイルを上書きする (overwrite_existing)", () => {
  ...
  // ファイルには id が埋め込まれる（リンク切れ時に id でファイルを探せるようにするため）。
  assert.equal(content.id, id);

  // 名前を変えたら Drive ファイル名も追従する（uniqueName.json へリネーム）。
...
test("Analytics_resolveQuestionRef_ はマッピングが壊れても name でファイルを探して再リンクする", () => {
  const ctx = loadAnalyticsContext();
  const saved = ctx.Analytics_saveTemplate_("questions", { name: "売上集計", query: { mode: "gui" } });
  const id = saved.question.id;
  breakQuestionMapping(ctx, id);

  const res = ctx.Analytics_resolveQuestionRef_({ questionId: id, name: "売上集計" });
  assert.equal(res.ok, true);
  ...
  // 埋め込み id を維持して再リンクし、マッピングを復元する。
  assert.equal(res.questionId, id);
  const restored = JSON.parse(ctx.__test.propsStore.get(QUESTIONS_MAPPING_KEY)).mapping;
  assert.ok(restored[id], "mapping が復元される");
});

test("Analytics_resolveQuestionRef_ は name 不一致でも埋め込み id でファイルを探して再リンクする", () => {
  const ctx = loadAnalyticsContext();
  const saved = ctx.Analytics_saveTemplate_("questions", { name: "orig", query: { mode: "gui" } });
  const id = saved.question.id;
  const fileId = JSON.parse(ctx.__test.propsStore.get(QUESTIONS_MAPPING_KEY)).mapping[id].fileId;
  // フォルダ外での手動改名を模擬（ファイル名を変える）＋ マッピング破壊。
  ctx.__test.fileStore.get(fileId).name = "totally-different.json";
  breakQuestionMapping(ctx, id);

  const res = ctx.Analytics_resolveQuestionRef_({ questionId: id, name: "no-such-name" });
  assert.equal(res.ok, true);
  assert.ok(res.question, "question を返す");
