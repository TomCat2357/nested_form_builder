...
test("Analytics_deleteTemplates_ はプロジェクト内は実体削除・プロジェクト外（外部リンク）はリンク解除のみ", () => {
  const ctx = loadAnalyticsContext();
  ...
  // プロジェクト内の実体はゴミ箱へ、プロジェクト外の実体は残る。
  assert.equal(ctx.__test.fileStore.get(insideId).trashed, true, "inside (project) file IS trashed");
