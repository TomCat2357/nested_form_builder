...
function loadAnalyticsContext() {
  ...
  const ctx = {
    ...
    // 本来は gas/codeHandlers.gs 定義。Analytics_saveTemplate_ / Analytics_deleteTemplates_ が
    // mapping の read-modify-write を排他するために使う。既定 shim はロック獲得成功（即 fn 実行）。
    // ロックタイムアウトの挙動はテスト内で ctx.WithScriptLock_ を差し替えて検証する。
...
// ---- lock: 並行更新の排他（ロックタイムアウト時は構造化エラーを返す） ----

// 実 WithScriptLock_（codeHandlers.gs）は tryLock 失敗時に BuildLockTimeoutResult_
// （{ ok:false, code:"LOCK_TIMEOUT", error }）を「投げずに返す」。Analytics_saveTemplate_ /
// Analytics_deleteTemplates_ はそれを nfbSafeCall_ 経由でそのまま透過する想定。
function withLockTimeout(ctx) {
  ctx.WithScriptLock_ = (label) => ({
    ok: false,
    code: "LOCK_TIMEOUT",
    error: `${label}処理は現在、他のユーザーによる更新中のため実行できませんでした。`,
  });
}

test("Analytics_saveTemplate_ はロックタイムアウト時に { ok:false, code:'LOCK_TIMEOUT' } を返す", () => {
  const ctx = loadAnalyticsContext();
  withLockTimeout(ctx);
  const res = ctx.Analytics_saveTemplate_("questions", { name: "q", query: { mode: "gui" } });
  assert.equal(res.ok, false);
  assert.equal(res.code, "LOCK_TIMEOUT");
  // 保存はロックで弾かれ mapping は作られない。
  assert.equal(ctx.__test.propsStore.has("nfb.analytics.questions.mapping"), false);
});

test("Analytics_deleteTemplates_ はロックタイムアウト時に { ok:false, code:'LOCK_TIMEOUT' } を返す", () => {
  const ctx = loadAnalyticsContext();
  // 先に1件保存してからロックを塞ぐ。
  const saved = ctx.Analytics_saveTemplate_("questions", { name: "q", query: { mode: "gui" } });
  const id = saved.question.id;
  withLockTimeout(ctx);
  const res = ctx.Analytics_deleteTemplates_("questions", [id]);
  assert.equal(res.ok, false);
  assert.equal(res.code, "LOCK_TIMEOUT");
  // ロックで弾かれ mapping は残ったまま（リンク解除されない）。
  const mapping = JSON.parse(ctx.__test.propsStore.get("nfb.analytics.questions.mapping")).mapping;
  assert.ok(mapping[id], "ロックタイムアウトで削除されず残る");
});
