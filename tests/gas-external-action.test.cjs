...
test("ExtAction_send_(handshakeSecret) は正規宛先へ 2 段階送信する（プローブに機微なし）", () => {
  const calls = [];
  const ctx = loadContext((url, opts) => {
    calls.push({ url, opts });
  ...
  const res = ctx.ExtAction_send_({ url: "https://x/exec", payload: { context: "record", secretData: "x" }, handshakeSecret: "S" });
  assert.equal(res.ok, true);
...
test("ExtAction_send_(handshakeSecret) は宛先を確認できないとき本データを送らない（DEST_UNVERIFIED）", () => {
  for (const probeBody of [
    ...
    const res = ctx.ExtAction_send_({ url: "https://wrong/exec", payload: { secretData: "x" }, handshakeSecret: "S" });
    assert.equal(res.ok, false);
