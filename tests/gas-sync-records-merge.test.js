...
test("Sync_syncFixedMetaColumnsFromRecord_ は同値時に固定メタ列の空セルだけ補完する", () => {
  ...
  assert.deepEqual(rowData[2], new Date(1700000000123));
  assert.equal(rowData[3], 2000);
  assert.deepEqual(rowData[4], new Date(1700000000999));
  ...
  // メタ日時列はシート上は数値の日時 (Date) を格納するため yyyy/mm/dd hh:mm:ss フォーマット
  assert.equal(rowFormats[1], "0");
  assert.equal(rowFormats[2], "yyyy/mm/dd hh:mm:ss");
  assert.equal(rowFormats[4], "yyyy/mm/dd hh:mm:ss");
});
...
test("Sync_syncFixedMetaColumnsFromRecord_ は modifiedAt が空欄または古い場合に固定メタ列をキャッシュで上書きする", () => {
  ...
  assert.equal(rowData[1], 12);
  assert.deepEqual(rowData[2], new Date(1700000000123));
  assert.deepEqual(rowData[3], new Date(1700000000999));
  assert.deepEqual(rowData[4], new Date(1700000000888));
  assert.equal(rowData[5], "creator@example.com");
  ...
  assert.equal(rowFormats[1], "0");
  assert.equal(rowFormats[2], "yyyy/mm/dd hh:mm:ss");
  assert.equal(rowFormats[3], "yyyy/mm/dd hh:mm:ss");
  assert.equal(rowFormats[4], "yyyy/mm/dd hh:mm:ss");
});
