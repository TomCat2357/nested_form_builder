...
test("Sync_syncFixedMetaColumnsFromRecord_ は同値時に固定メタ列の空セルだけ補完する", () => {
  ...
  assert.equal(changed, true);
  assert.equal(rowData[2], 12);
  assert.equal(rowData[3], 1700000000123);
  assert.equal(rowData[4], 2000);
  assert.equal(rowData[5], 1700000000999);
  assert.equal(rowData[6], "creator@example.com");
  assert.equal(rowData[7], "modifier@example.com");
  assert.equal(rowData[8], "deleter@example.com");
  assert.equal(rowData[9], "https://drive.google.com/drive/folders/folder_a");
  assert.equal(rowFormats[2], "0");
  assert.equal(rowFormats[3], "0");
  assert.equal(rowFormats[5], "0");
});
...
test("Sync_syncFixedMetaColumnsFromRecord_ は同値時に既存の固定メタ列を上書きしない", () => {
  ...
  assert.equal(changed, false);
  assert.equal(rowData[2], 3);
  assert.equal(rowData[3], 1700000000001);
  assert.equal(rowData[5], 1700000000003);
  assert.equal(rowData[6], "existing-creator@example.com");
  assert.equal(rowData[7], "existing-modifier@example.com");
  assert.equal(rowData[8], "existing-deleter@example.com");
  assert.equal(rowData[9], "https://drive.google.com/drive/folders/existing");
});
...
test("Sync_syncFixedMetaColumnsFromRecord_ は modifiedAt が空欄または古い場合に固定メタ列をキャッシュで上書きする", () => {
  ...
  assert.equal(rowData[5], 1700000000888);
  assert.equal(rowData[6], "creator@example.com");
  assert.equal(rowData[7], "cache-modifier@example.com");
  assert.equal(rowData[8], "cache-deleter@example.com");
  assert.equal(rowData[9], "https://drive.google.com/drive/folders/cache");
  assert.equal(rowFormats[2], "0");
  assert.equal(rowFormats[3], "0");
