 ...
 * （{UPPER(`field`)} 等）に統一された。ここでは expressionEvaluator.gs +
 * templateEvaluator.gs を組み込んだ driveTemplate.gs の挙動を、Google Doc
 * テンプレート差し込み・ファイル名解決・フォルダ自動命名まで含めて確認する。
 */
...
test("nfbResolveTemplateTokens_: {} は dataValues（元データ形式）、{{}} は fieldValues（ビュー形式）を引く", () => {
  const gas = loadGasContext();
  const ctx = {
    fieldPaths: { g: "性別" },
    fieldValues: { g: "男" },          // ビュー形式: ラベル
    dataValues: { "性別|男": true },   // 元データ形式: オプション単位の真偽値
    responses: {},
    now: new Date("2026-04-04T10:20:30+09:00"),
  };
  // 単一ブレース → dataValues（オプション単位パス）
  assert.equal(gas.nfbResolveTemplateTokens_("{`性別|男`}", ctx), "true");
  // 二重ブレース → fieldValues（フィールドパス＝ラベル連結）
  assert.equal(gas.nfbResolveTemplateTokens_("{{`性別`}}", ctx), "男");
  // 混在
  assert.equal(
    gas.nfbResolveTemplateTokens_("元:{`性別|男`}/ビュー:{{`性別`}}", ctx),
    "元:true/ビュー:男",
  );
});

test("nfbResolveTemplateTokens_: dataValues 未指定なら {} は従来どおり fieldValues にフォールバック", () => {
  const gas = loadGasContext();
  const ctx = {
    fieldPaths: { f1: "氏名" },
...
test("nfbFindDriveFileInFolder は PDF 出力時に .pdf を補完して検索する", () => {
  ...
  let searchedName = "";
  const foundFile = {
    getUrl() { return "https://drive.google.com/file/d/pdf001/view"; },
    getName() { return "rec001_output.pdf"; },
    getId() { return "pdf001"; },
  };
  const folder = {
    getFilesByName(fileName) {
      ...
      let used = false;
      return {
        hasNext() { return !used && fileName === "rec001_output.pdf"; },
        next() { used = true; return foundFile; },
      };
    },
    getUrl() { return "https://drive.google.com/drive/folders/folder123"; },
  };
