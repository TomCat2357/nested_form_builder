...
function loadGasContext() {
  ...
  const context = {
    ...
    // 本体は driveBrowser.gs。このテストは driveBrowser.gs を読み込まないためスタブする。
    // false を返すことで nfbMakeDriveFileCopy_ の DriveApp.makeCopy フォールバック経路
    // （= 各テストが mock している makeCopy / getFileById）を通す。
    DriveBrowser_isDriveAdvancedAvailable_() { return false; },
  };
...
test("nfbCopyDriveFileToDrive は高度サービスがあれば supportsAllDrives 付き Drive.Files.copy を使う（共有ドライブ対応）", () => {
  const gas = loadGasContext();
  const existingFile = {
    trashed: false,
    setTrashed(v) { this.trashed = v; },
  ...
  const folder = {
    getId() { return "folder123"; },
    getUrl() { return "https://drive.google.com/drive/folders/folder123"; },
    getName() { return "record_tmp"; },
    getFilesByName(fileName) {
      let used = false;
      return {
        hasNext() { return !used && fileName === "ソース名.pdf"; },
        next() { used = true; return existingFile; },
      };
    },
  };
  const copiedFile = {
    getId() { return "copied123"; },
    getUrl() { return "https://drive.google.com/file/d/copied123/view"; },
    getName() { return "ソース名.pdf"; },
  };
  let copyArgs = null;

  // 高度サービスありの分岐に入れる。DriveApp.makeCopy は使われない。
  gas.DriveBrowser_isDriveAdvancedAvailable_ = function() { return true; };
  gas.Drive = {
    Files: {
      get(id, opts) {
        assert.equal(id, "src123");
        assert.equal(opts.supportsAllDrives, true);
        return { name: "ソース名.pdf" };
      },
      copy(resource, fileId, opts) {
        copyArgs = { resource, fileId, opts };
        return { id: "copied123" };
      },
    },
  };
  gas.DriveApp = { getFileById(id) { assert.equal(id, "copied123"); return copiedFile; } };
  gas.nfbResolveUploadFolder_ = function() { return { folder, autoCreated: false }; };

  const result = gas.nfbCopyDriveFileToDrive({
    sourceUrl: "https://drive.google.com/file/d/src123/view",
    driveSettings: {},
  });

  // 共有ドライブのソース／コピー先で確実に動くよう supportsAllDrives を明示しているか
  // （resource は vm 内で生成されるため deepStrictEqual を避け、プロパティ単位で検証する）
  assert.equal(copyArgs.fileId, "src123");
  assert.equal(copyArgs.resource.name, "ソース名.pdf");
  assert.equal(copyArgs.resource.parents.length, 1);
  assert.equal(copyArgs.resource.parents[0], "folder123");
  assert.equal(copyArgs.opts.supportsAllDrives, true);
  // 同名上書き（trash）→ コピーの順序が保たれているか
  assert.equal(existingFile.trashed, true);
  assert.equal(result.ok, true);
  assert.equal(result.fileId, "copied123");
  assert.equal(result.fileName, "ソース名.pdf");
});
...
test("nfbResolveTemplateUrlPhysicalFirst_: 物理 fileId が死んでいれば論理パスで 05 を引き直す（物理優先・論理フォールバック）", () => {
  const gas = loadGasContext();
  // card123 のみ生存。dead999 は削除済み（ゴミ箱）を模す。
  const aliveIds = { card123: true };
  gas.Nfb_isFileIdAliveCached_ = (fileId) => !!aliveIds[fileId];
  gas.StdFolders_resolvePathToFileId_ = (key, p) =>
    (key === "report_templates" && p === "様式/出力Doc") ? "recovered777" : "";

  // 物理 fileId 生存 → そのまま URL 化して採用（論理は見ない）
  assert.equal(
    gas.nfbResolveTemplateUrlPhysicalFirst_("card123", "様式/出力Doc"),
    "https://docs.google.com/document/d/card123/edit"
  );

  // 物理 fileId が死亡 + 論理パスあり → 05 を引き直した URL（保存時の生存検証に揃える）
  assert.equal(
    gas.nfbResolveTemplateUrlPhysicalFirst_("dead999", "様式/出力Doc"),
    "https://docs.google.com/document/d/recovered777/edit"
  );

  // 物理 fileId が死亡 + 論理パス無し → フォールバック先が無いので生存検証を省いて従来どおり fileId を URL 化
  assert.equal(
    gas.nfbResolveTemplateUrlPhysicalFirst_("dead999", ""),
    "https://docs.google.com/document/d/dead999/edit"
  );

  // 物理 fileId 死亡 + 論理も引けない → 元の fileId 由来 URL（最後の手段）
  assert.equal(
    gas.nfbResolveTemplateUrlPhysicalFirst_("dead999", "存在しない/パス"),
    "https://docs.google.com/document/d/dead999/edit"
  );

  // fileId 空 + 論理パスあり → 論理で合成
  assert.equal(
    gas.nfbResolveTemplateUrlPhysicalFirst_("", "様式/出力Doc"),
    "https://docs.google.com/document/d/recovered777/edit"
  );
});
