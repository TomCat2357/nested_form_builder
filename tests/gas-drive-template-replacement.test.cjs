...
test("subtemplate: ifv 真の値でフィールド参照とリテラル文字列を組み合わせ", () => {
  ...
  assert.equal(
    gas.nfbResolveTemplate_("{@状態|ifv:@状態==完了,({@対応者})記載あり,記載なし}", ctx),
    "(山田)記載あり"
  );
...
test("subtemplate: default 引数でフィールド参照とリテラル文字列を組み合わせ", () => {
  ...
  assert.equal(
    gas.nfbResolveTemplate_("{@納期|default:未定（{@登録日|time:M月D日}時点）}", ctx),
    "未定（4月4日時点）"
  );
...
test("subtemplate: if else値でサブテンプレート", () => {
  ...
  assert.equal(
    gas.nfbResolveTemplate_("{@備考|if:@備考,（{@氏名}より）}", ctx),
    "（佐藤より）"
  );
});
...
test("subtemplate: {_} と {@_} はサブテンプレート内でパイプ入力値を返す", () => {
  ...
  assert.equal(
    gas.nfbResolveTemplate_("{@報告|ifv:@報告,({_})完了,未完了}", ctx),
    "(受領)完了"
  );
  assert.equal(
    gas.nfbResolveTemplate_("{@報告|ifv:@報告,({@_})完了,未完了}", ctx),
...
test("subtemplate: トップレベルの {_} は空文字（パイプ値なし）", () => {
  ...
  assert.equal(gas.nfbResolveTemplate_("{_}", ctx), "");
});
...
test("subtemplate: ifv のサブテンプレートでパイプ変換チェーン", () => {
  ...
  assert.equal(
    gas.nfbResolveTemplate_("{@ステータス|ifv:@ステータス==確定,¥{@金額|number:#,##0},未確定}", ctx),
    "¥1,500"
  );
});
...
test("subtemplate: \\{ \\} エスケープがサブテンプレート内でも機能する", () => {
  ...
  assert.equal(
    gas.nfbResolveTemplate_("{@名前|ifv:@名前,\\{{@名前}\\},未入力}", ctx),
    "{太郎}"
  );
});
...
test("subtemplate: 未閉鎖ブレースは安全に文字列扱い", () => {
  ...
  assert.equal(gas.nfbResolveTemplate_("prefix {未閉鎖", ctx), "prefix {未閉鎖");
});
...
test("subtemplate: replace の to が {} を含む値でもコンマ位置を正しく解釈", () => {
  ...
  assert.equal(
    gas.nfbResolveTemplate_("{@テキスト|replace:-,{x}}", ctx),
    "X{x}Y{x}Z"
  );
});
