...
test("nfbApplyTemplateReplacementsToGoogleDocument_ は ifv のサブテンプレート（ネストした{}）を正しく解決する", () => {
  const gas = loadGasContext();

  const withValue = createTextElement("報告{@報道（予定）等|ifv:_,（{@報道（予定）等}）,}");
  const withoutValue = createTextElement("報告{@報道（予定）等|ifv:_,（{@報道（予定）等}）,}");
  const usingUnderscore = createTextElement("状態{@報道（予定）等|ifv:_,（{_}）,}");
...
test("パイプ変換: if で == 等値比較", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@性別|if:@性別==男,Ms.}", ctx), "男");
  // 条件不一致 → else値
  ctx.fieldValues.g = "女";
  assert.equal(gas.nfbResolveTemplateTokens_("{@性別|if:@性別==男,Ms.}", ctx), "Ms.");
});
...
test("パイプ変換: if で truthiness 判定", () => {
  ...
  // 値あり → パイプ入力値を返す
  assert.equal(gas.nfbResolveTemplateTokens_("{@備考|if:@備考,なし}", ctx), "あり");
  // 値なし → else値
  ctx.fieldValues.f = "";
  assert.equal(gas.nfbResolveTemplateTokens_("{@備考|if:@備考,なし}", ctx), "なし");
});
...
test("if: truthiness で他フィールド参照（参照先に値あり）", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@報告者一覧|if:@報告者一覧,@報告者名}", ctx), "田中,鈴木");
});
...
test("if: truthiness で他フィールド参照（参照先が空 → else の @フィールド解決）", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@報告者一覧|if:@報告者一覧,@報告者名}", ctx), "山田");
});
...
test("if: not で否定 truthiness", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@報告者一覧|if:not @報告者一覧,aaa}", ctx), "");
  // 報告者一覧に値あり → not で false → else値 "aaa"
  ctx.fieldValues.a = "田中";
  assert.equal(gas.nfbResolveTemplateTokens_("{@報告者一覧|if:not @報告者一覧,aaa}", ctx), "aaa");
});
...
test("if: == 等値比較", () => {
  ...
  // 一致 → パイプ入力値を返す
  assert.equal(gas.nfbResolveTemplateTokens_("{@報告者一覧|if:@報告者一覧==aaa,bbb}", ctx), "aaa");
  // 不一致 → else値
  ctx.fieldValues.a = "xxx";
  assert.equal(gas.nfbResolveTemplateTokens_("{@報告者一覧|if:@報告者一覧==aaa,bbb}", ctx), "bbb");
});
...
test("if: == クォート付きリテラル比較", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_('{@性別|if:@性別=="男",女}', ctx), "男");
  ctx.fieldValues.a = "女";
  assert.equal(gas.nfbResolveTemplateTokens_('{@性別|if:@性別=="男",女}', ctx), "女");
});
...
test("if: != 不等比較", () => {
  ...
  // "完了" != "未完了" → true → パイプ入力値
  assert.equal(gas.nfbResolveTemplateTokens_("{@状態|if:@状態!=未完了,未完了}", ctx), "完了");
  ctx.fieldValues.a = "未完了";
  assert.equal(gas.nfbResolveTemplateTokens_("{@状態|if:@状態!=未完了,未完了}", ctx), "未完了");
});
...
test("if: > >= < <= 数値比較", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@金額|if:@金額>1000,低額}", ctx), "1500");
  assert.equal(gas.nfbResolveTemplateTokens_("{@金額|if:@金額<1000,高額}", ctx), "高額");
  assert.equal(gas.nfbResolveTemplateTokens_("{@金額|if:@金額>=1500,未満}", ctx), "1500");
  assert.equal(gas.nfbResolveTemplateTokens_("{@金額|if:@金額<=1000,超過}", ctx), "超過");
});
...
test("if: フィールド対フィールド比較", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@金額|if:@金額>@基準額,基準以下}", ctx), "1500");
  ctx.fieldValues.a = "500";
  assert.equal(gas.nfbResolveTemplateTokens_("{@金額|if:@金額>@基準額,基準以下}", ctx), "基準以下");
});
...
test("if: in 演算子（文字列包含チェック）", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@結果|if:@結果 in @結果一覧,対象外}", ctx), "合格");
  ctx.fieldValues.a = "欠席";
  assert.equal(gas.nfbResolveTemplateTokens_("{@結果|if:@結果 in @結果一覧,対象外}", ctx), "対象外");
});
...
test("if: in 演算子でリテラル検索", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_('{@メンバー一覧|if:"鈴木" in @メンバー一覧,なし}', ctx), "田中,鈴木,佐藤");
  assert.equal(gas.nfbResolveTemplateTokens_('{@メンバー一覧|if:"山田" in @メンバー一覧,なし}', ctx), "なし");
});
...
test("if: not と比較演算子の組み合わせ", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@金額|if:not @金額>1000,高額}", ctx), "高額");
});
...
test("if: else値で _ プレースホルダ（パイプ入力値）", () => {
  ...
  // 条件不一致 → _ でパイプ入力値を返す
  assert.equal(gas.nfbResolveTemplateTokens_("{@名前|if:@候補,_}", ctx), "山田太郎");
  // 条件一致 → パイプ入力値
  ctx.fieldValues.b = "";
  assert.equal(gas.nfbResolveTemplateTokens_("{@名前|if:@候補,_}", ctx), "山田太郎");
});
...
test("if: else値で \\_ リテラルアンダースコア", () => {
  ...
  // 条件不一致 → \_ でリテラル "_" を返す
  assert.equal(gas.nfbResolveTemplateTokens_("{@名前|if:@名前==不一致,\\_}", ctx), "_");
});
...
test("if: 存在しないフィールド参照は空扱い", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@報告者役職|if:@存在しない,_}", ctx), "部長");
});
...
test("@ prefix: フィールド参照は @ 必須", () => {
  ...
  // @ なし → トークンとして解決されず空文字
  assert.equal(gas.nfbResolveTemplateTokens_("{名前}", ctx), "");
});
...
test("@ prefix: 予約トークンは @ 必須", () => {
  ...
  // @ なしでは予約トークンとして解決されない
  assert.equal(gas.nfbResolveTemplateTokens_("{_id}", ctx), "");
});
...
test("ifv: 基本 — 条件一致で真の値、不一致で偽の値", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@対応方法|ifv:@対応方法==来庁,■,□}", ctx), "■");
...
test("ifv: in 演算子でチェックボックス部分一致", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@報道の結果|ifv:記事掲載 in _,■,□}", ctx), "■");
  assert.equal(gas.nfbResolveTemplateTokens_("{@報道の結果|ifv:放送予定 in _,■,□}", ctx), "□");
  assert.equal(gas.nfbResolveTemplateTokens_("{@報道の結果|ifv:その他 in _,■,□}", ctx), "■");
  assert.equal(gas.nfbResolveTemplateTokens_("{@報道の結果|ifv:報道なし in _,■,□}", ctx), "□");
});
...
test("ifv: 真の値に _ でパイプ入力値を返す", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@名前|ifv:@名前,_,不明}", ctx), "山田太郎");
  ctx.fieldValues.a = "";
  assert.equal(gas.nfbResolveTemplateTokens_("{@名前|ifv:@名前,_,不明}", ctx), "不明");
});
...
test("ifv: 真の値・偽の値に @ref でフィールド参照解決", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@状態|ifv:@状態==完了,@完了メッセージ,@未完了メッセージ}", ctx), "OK");
  ctx.fieldValues.a = "未完了";
  assert.equal(gas.nfbResolveTemplateTokens_("{@状態|ifv:@状態==完了,@完了メッセージ,@未完了メッセージ}", ctx), "NG");
});
...
test("ifv: not 否定との組み合わせ", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@備考|ifv:not @備考,空です,値あり}", ctx), "値あり");
  ctx.fieldValues.a = "";
  assert.equal(gas.nfbResolveTemplateTokens_("{@備考|ifv:not @備考,空です,値あり}", ctx), "空です");
});
...
test("ifv: 他パイプとのチェーン", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@金額|ifv:@金額>1000,_,0|number:#,##0}", ctx), "1,500");
});
...
test("ifv: カンマ不足（引数1つ以下）はパイプ入力値をそのまま返す", () => {
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{@名前|ifv:@名前}", ctx), "太郎");
  assert.equal(gas.nfbResolveTemplateTokens_("{@名前|ifv:@名前,OK}", ctx), "太郎");
});
...
test("subtemplate: ifv 真の値でフィールド参照とリテラル文字列を組み合わせ", () => {
  const gas = loadGasContext();
  const ctx = {
  ...
  assert.equal(
    gas.nfbResolveTemplateTokens_("{@状態|ifv:@状態==完了,({@対応者})記載あり,記載なし}", ctx),
    "(山田)記載あり"
  );
...
test("subtemplate: if else値でサブテンプレート", () => {
  ...
  assert.equal(
    gas.nfbResolveTemplateTokens_("{@備考|if:@備考,（{@氏名}より）}", ctx),
    "（佐藤より）"
  );
});
...
test("subtemplate: {_} と {@_} はサブテンプレート内でパイプ入力値を返す", () => {
  ...
  assert.equal(
    gas.nfbResolveTemplateTokens_("{@報告|ifv:@報告,({_})完了,未完了}", ctx),
    "(受領)完了"
  );
  assert.equal(
    gas.nfbResolveTemplateTokens_("{@報告|ifv:@報告,({@_})完了,未完了}", ctx),
...
test("subtemplate: トップレベルの {_} は空文字（パイプ値なし）", () => {
  const gas = loadGasContext();
  const ctx = {
    fieldLabels: {},
    fieldValues: {},
  ...
  assert.equal(gas.nfbResolveTemplateTokens_("{_}", ctx), "");
});

test("subtemplate: ifv のサブテンプレートでパイプ変換チェーン", () => {
  const gas = loadGasContext();
  const ctx = {
  ...
  assert.equal(
    gas.nfbResolveTemplateTokens_("{@ステータス|ifv:@ステータス==確定,¥{@金額|number:#,##0},未確定}", ctx),
    "¥1,500"
  );
});

test("subtemplate: 2段ネストの ifv", () => {
  const gas = loadGasContext();
  const ctx = {
...
test("subtemplate: サブテンプレート内のコンマで誤分割されない", () => {
  ...
  // {@値|map:1=A;2=B;*=C} の中のセミコロンや、ifv 引数の中の {} はコンマで誤分割されない
  assert.equal(
    gas.nfbResolveTemplateTokens_(
      "{@値|ifv:@値,選択:{@値|map:1=甲;2=乙;*=その他},{@ラベル}}",
      ctx
    ),
    "選択:甲"
  );
...
test("subtemplate: \\{ \\} エスケープがサブテンプレート内でも機能する", () => {
  ...
  assert.equal(
    gas.nfbResolveTemplateTokens_("{@名前|ifv:@名前,\\{{@名前}\\},未入力}", ctx),
    "{太郎}"
  );
});
