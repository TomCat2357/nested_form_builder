...
test("normalizePrintTemplateRefsOnSave_: フォーム全体 + カード個別を 05 へ寄せ id/path を両方書き戻し旧 URL を剥がす", () => {
  const gas = loadCtx();
  // 任意の参照を新 fileId/path へ寄せたとみなす（実 Drive 非依存）。
  gas.StdFolders_alignFileRefIntoStdFolder_ = (key, idOrUrl, path) => ({
  ...
  assert.equal(form.settings.standardPrintTemplateId, "X");
  assert.equal("standardPrintTemplateUrl" in form.settings, false, "旧 URL キーを剥がす");
  assert.equal(form.settings.standardPrintTemplatePath, "NEW_PATH");
  assert.equal(form.schema[0].printTemplateAction.templateId, "X");
  assert.equal("templateUrl" in form.schema[0].printTemplateAction, false, "旧 URL キーを剥がす");
  assert.equal(form.schema[0].printTemplateAction.templatePath, "NEW_PATH");
  assert.equal(form.schema[0].children[0].printTemplateAction.templateId, "X", "ネスト配下も走査される");
