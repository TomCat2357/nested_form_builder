...
// 埋め込み id インデックス、構成内コピー（既存正規化）を差し替える。
//   aliveIds   : 生存している fileId の集合
//   indexByKey : key → { 埋め込みid: { fileId, fileUrl } }
function stubRepairEnv(gas, { aliveIds = [], indexByKey = {} } = {}) {
  const alive = new Set(aliveIds);
  gas.StdFolders_isFileIdAlive_ = (fileId) => alive.has(fileId);
  gas.StdFolders_indexStdFolderByEmbeddedId_ = (key) => indexByKey[key] || {};
  // 生存ファイルは「既に構成内」とみなし、コピーによる正規化は発生させない。
  gas.StdFolders_ensureMappingEntryInStd_ = () => ({ changed: false });
}

test("StdFolders_repairAndNormalizeMapping_: 壊れたリンクは埋め込み id 一致先へ再リンクする", () => {
  const gas = loadGasContext();
  stubRepairEnv(gas, {
    aliveIds: [],
    indexByKey: { forms: { f1: { fileId: "NEWF1", fileUrl: "https://drive.google.com/file/d/NEWF1/view" } } },
  });
  ...
  assert.equal(mapping.f1.fileId, "NEWF1");
  assert.equal(mapping.f1.driveFileUrl, "https://drive.google.com/file/d/NEWF1/view");
});
...
test("StdFolders_normalizeLinkedToStd_: 再リンク時は AddFormUrl_ も更新し、合計件数を返す", () => {
  ...
  stubRepairEnv(gas, {
    aliveIds: ["LIVEQ"],
    indexByKey: {
      forms: { f1: { fileId: "NEWF1", fileUrl: "https://drive.google.com/file/d/NEWF1/view" } },
      questions: {},
      dashboards: {},
    },
  });
  const stores = installStores(gas, {
    forms: { f1: { fileId: "DEADF1", driveFileUrl: "old", title: "T" } }, // 壊れ → 再リンク
  ...
  assert.equal(res.relinked, 1);
  assert.equal(res.removed, 1);
  assert.equal(stores.forms.f1.fileId, "NEWF1");
