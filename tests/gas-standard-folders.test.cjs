...
test("StdFolders_rewireDashboardFile_: questionId を idMap で新 fileId へ再マップ、未収載は空にし questionPath を保持・未解決として数える", () => {
  const gas = loadGasContext();
  const before = {
    cards: [
  ...
  // 収載済みは新 fileId へ再マップ。論理パスは据え置き。
  assert.equal(after.cards[0].questionId, "Q_NEW_KEPT");
  assert.equal(after.cards[0].questionPath, "02_questions/売上", "収載済みの論理パスは据え置き");
  // 未収載はコピー元へ残さず空にし、論理パス（questionPath）を復旧アンカーとして保持。
  assert.equal(after.cards[1].questionId, "", "未収載はコピー元へ残さず空にする");
  assert.equal(after.cards[1].questionPath, "02_questions/離脱率", "未収載でも論理パスは保持");
  assert.equal(after.cards[1].questionName, "離脱率");
});

test("StdFolders_rewireQuestionFile_: formId を idMap で新 fileId へ再マップ、未収載は空にし formPath を保持・未解決数を返す", () => {
  const gas = loadGasContext();
  const before = {
    query: {
      mode: "gui",
  ...
  assert.equal(after.query.gui.formPath, "01_forms/売上", "論理パスは据え置き");
  assert.equal(after.query.formSources[0].formId, "F_NEW");
  assert.equal(after.query.formSources[1].formId, "", "idMap に無い formId はコピー元へ残さず空にする");
  assert.equal(after.query.formSources[1].formPath, "01_forms/外", "未収載でも論理パスは保持");
});

test("StdFolders_rewireFormFile_: formLink の childFormId を idMap で再マップ（ネスト含む）、未収載は空にし childFormPath を保持・未解決を返す", () => {
  const gas = loadGasContext();
  const before = {
    settings: { spreadsheetId: "https://docs.google.com/spreadsheets/d/SHEETID1234567890/edit" },
    schema: [
  ...
  assert.equal(res.cleared, 0, "URL 系のクリアは無し");
  assert.equal(res.unresolved, 1, "未収載の子フォームリンク 1 件を未解決として数える");
  const after = JSON.parse(state.content);
  // ネスト配下（section.children）の formLink も walkFields の再帰で再配線される。
  const links = after.schema[0].children;
  assert.equal(links[0].childFormId, "CF_NEW", "収載済みは新 fileId へ");
  assert.equal(links[0].childFormPath, "01_forms/子A", "論理パスは据え置き");
  assert.equal(links[1].childFormId, "", "未収載はコピー元へ残さず空にする");
  assert.equal(links[1].childFormPath, "01_forms/子B", "未収載でも論理パスは保持");
  // URL 系（spreadsheet）の再マップは従来どおり。
  assert.equal(after.settings.spreadsheetId, "https://docs.google.com/spreadsheets/d/NEWSS/edit");
});

test("StdFolders_rewireFormFile_: settings.spreadsheetPath は触らず保持（直接 id は remap/クリア）", () => {
  const gas = loadGasContext();
  const before = {
    settings: { spreadsheetPath: "売上/集計2026", spreadsheetId: "" },
  ...
  assert.equal(res.cleared, 0, "論理パスはクリア対象に数えない");
...
test("StdFolders_copy_: 親フォームの childFormId(formLink) がコピー先の子フォーム fileId へ張り替わる", () => {
  const gas = loadGasContext();
  ...
  assert.equal(res.unresolvedLinks, 0, "親子ともコピー対象なので未解決なし");
  const parentJson = JSON.parse(env.registry["COPY_PARENT"]._content);
  assert.equal(parentJson.schema[0].childFormId, "COPY_CHILD", "子フォームリンクがコピー先の子フォームを指す");
  assert.equal(parentJson.schema[0].childFormPath, "01_forms/子", "論理パスは据え置き");
});
